...
try {
    ...
    $melazaQuery = "SELECT 
                        ...
                        AND ch_melaza_racion IS NOT NULL 
                        AND ch_melaza_costo IS NOT NULL
                        ORDER BY ch_melaza_fecha_inicio ASC";
    ...
    if (empty($melazaData)) {
        ...
        $melazaQuery = "SELECT 
                                ch_melaza_fecha,
                                ch_melaza_fecha as ch_melaza_fecha_inicio,
                                ch_melaza_fecha as ch_melaza_fecha_fin,
                                ch_melaza_racion,
                                ch_melaza_costo,
                                ch_melaza_tagid
                            FROM ch_melaza 
    ...
    foreach ($melazaData as $melaza) {
        $startDate = new DateTime($melaza['ch_melaza_fecha_inicio']);
        $endDate = new DateTime($melaza['ch_melaza_fecha_fin']);
        
        // Calculate total expense value
        $totalDays = $endDate->diff($startDate)->days + 1;
        $totalValue = $melaza['ch_melaza_racion'] * $melaza['ch_melaza_costo'] * $totalDays;
        $dailyValue = $totalValue / $totalDays;
        
        error_log("Processing melaza: Start=" . $melaza['ch_melaza_fecha_inicio'] . 
                 ", End=" . $melaza['ch_melaza_fecha_fin'] . 
                 ", Days=" . $totalDays . 
...
<title>caprino - Registro de Melaza</title>
<!-- Link to the Favicon -->
...
<div class="container text-center">
                        ...
                        <div class="input-group">
                            <span class="input-group-text">
                                ...
                                <select class="form-select" id="new_alimento" name="alimento" required>
                                    ...
                                    <?php
                                    try {
                                        $sql_alimentos = "SELECT DISTINCT cc_melaza_nombre FROM cc_melaza ORDER BY cc_melaza_nombre ASC";
                                        ...
                                        foreach ($alimentos as $alimento_row) {
                                            echo '<option value="' . htmlspecialchars($alimento_row['cc_melaza_nombre']) . '">' . htmlspecialchars($alimento_row['cc_melaza_nombre']) . '</option>';
                                        }
                                    } catch (PDOException $e) {
  ...
  <!-- DataTable for ch_melaza records -->
              ...
              <tbody>
                  ...
                  try {
                      // Query to get all Animals and ALL their melaza records (if any)
                        $melazaQuery = "
                            SELECT
                                ...
                                c.ch_melaza_fecha_inicio,
                                c.ch_melaza_fecha_fin,
                                c.ch_melaza_tagid,         -- Matches caprino_tagid if melaza exists
                                c.ch_melaza_etapa,
                                c.ch_melaza_producto,
                                c.ch_melaza_racion,
                                c.ch_melaza_costo,
                                -- Calculate total_value only if c.id is not null
                                CASE WHEN c.id IS NOT NULL THEN CAST((c.ch_melaza_racion * c.ch_melaza_costo) AS DECIMAL(10,2)) ELSE NULL END as total_value
                            FROM
                                caprino v
                            LEFT JOIN
                                ch_melaza c ON v.tagid = c.ch_melaza_tagid -- Join ALL matching melaza records
                            ORDER BY
                      ...
                      if (empty($melazaData)) {
                          ...
                          try {
                              $configQuery = "SELECT c_vencimiento_melaza FROM c_vencimiento LIMIT 1";
                              $configStmt = $conn->prepare($configQuery);
                              $configStmt->execute();
                              
                              // Explicitly use PDO fetch method
                              $row = $configStmt->fetch(PDO::FETCH_ASSOC);
                              if ($row && isset($row['c_vencimiento_melaza'])) {
                                  $vigencia = intval($row['c_vencimiento_melaza']);
                              }
                          } catch (PDOException $e) {
                          ...
                          foreach ($melazaData as $row) {
                              ...
                              $melazaFecha = $row['ch_melaza_fecha_inicio'] ?? null;
                              $melazaFechaInicio = $row['ch_melaza_fecha_inicio'] ?? null;
                              $melazaFechaFin = $row['ch_melaza_fecha_fin'] ?? null;
                              ...
                              if ($hasMelaza) {
                                  ...
                                  echo '        <button class="btn btn-warning btn-sm edit-melaza" 
                                                  ...
                                                  data-producto="'.htmlspecialchars($row['ch_melaza_producto'] ?? '').'" 
                                                  data-racion="'.htmlspecialchars($row['ch_melaza_racion'] ?? '').'" 
                                                  data-costo="'.htmlspecialchars($row['ch_melaza_costo'] ?? '').'" 
                                                  data-fecha-inicio="'.htmlspecialchars($melazaFechaInicio ?? '').'"
                                                  data-fecha-fin="'.htmlspecialchars($melazaFechaFin ?? '').'"
                                                  title="Editar Registro"
                                                  data-debug="DB: '.htmlspecialchars($row['ch_melaza_producto'] ?? 'NULL').'">
                              ...
                              echo "<td>" . ($hasMelaza ? htmlspecialchars($row['ch_melaza_producto'] ?? '') : 'N/A') . "</td>";
                              // Column 7: Racion (or N/A)
                              echo "<td>" . ($hasMelaza ? htmlspecialchars($row['ch_melaza_racion'] ?? '') : 'N/A') . "</td>";
                              // Column 8: Costo (or N/A)
                              echo "<td>" . ($hasMelaza ? htmlspecialchars($row['ch_melaza_costo'] ?? '') : 'N/A') . "</td>";
                        ...
                        <div class="mb-3">
                            ...
                            <div class="list-group" id="productList" style="max-height: 300px; overflow-y: auto;">
                                ...
                                try {
                                    ...
                                    $alimentos = $stmt_alimentos->fetchAll(PDO::FETCH_ASSOC);
                                    foreach ($alimentos as $alimento_row) {
                                        echo '<button type="button" class="list-group-item list-group-item-action product-option" data-value="' . htmlspecialchars($alimento_row['cc_melaza_nombre']) . '">' . htmlspecialchars($alimento_row['cc_melaza_nombre']) . '</button>';
                                    }
                                } catch (PDOException $e) {
