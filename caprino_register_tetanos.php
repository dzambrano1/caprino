...
<title>Caprino - Registro de Vacunación Tetanos</title>
<!-- Link to the Favicon -->
...
<div class="container text-center">
  ...
  REGISTROS VACUNACION TETANOS
  </h3>
  
  <!-- New Tetanos Entry Modal -->
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="newEntryModalLabel">
                    <i class="fas fa-plus-circle me-2"></i>Nueva Vacunacion Tetanos
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form id="newTetanosForm">
                <div class="mb-4">
                    ...
                    <div class="mb-4">                        
                        <div class="input-group">
                            <span class="input-group-text">
                                ...
                                <label for="new_vacuna" class="form-label">Vacunas Tetanos</label>
                                <select class="form-select" id="new_vacuna" name="vacuna" required>
                                    <option value="">Vacunas</option>
                                    <?php
                                    try {
                                        $sql_vacunas = "SELECT DISTINCT cc_tetanos_vacuna FROM cc_tetanos ORDER BY cc_tetanos_vacuna ASC";
                                        ...
                                        foreach ($vacunas as $vacuna_row) {
                                            echo '<option value="' . htmlspecialchars($vacuna_row['cc_tetanos_vacuna']) . '">' . htmlspecialchars($vacuna_row['cc_tetanos_vacuna']) . '</option>';
                                        }
            ...
            <div class="modal-footer btn-group">
                ...
                <button type="button" class="btn btn-success" id="saveNewTetanos">
  ...
  <!-- DataTable for ch_tetanos records -->
  
  <div class="container table-section" style="display: block;">
      <div class="table-responsive">
          <table id="tetanosTable" class="table table-striped table-bordered">
              <thead>
                  <tr>
              ...
              <tbody>
                  ...
                  try {
                      $tetanosQuery = "SELECT 
                                          ...
                                          v.nombre,
                                          va.ch_tetanos_fecha,
                                          va.id AS tetanos_record_id, -- Alias tetanos record ID
                                          va.ch_tetanos_producto,
                                          va.ch_tetanos_dosis,
                                          va.ch_tetanos_costo,
                                          -- Flag to easily check if there's a history record
                                          CASE WHEN va.ch_tetanos_tagid IS NOT NULL THEN 1 ELSE 0 END AS in_tetanos_history
                                      FROM 
                                          caprino v
                                      LEFT JOIN 
                                          -- Join with all tetanos records per tagid
                                          ch_tetanos va ON v.tagid = va.ch_tetanos_tagid 
                                      ORDER BY 
                                          va.ch_tetanos_fecha DESC, v.tagid ASC;";

                      $stmt = $conn->prepare($tetanosQuery);
                      $stmt->execute();
                      $tetanossData = $stmt->fetchAll(PDO::FETCH_ASSOC);

                      if (empty($tetanossData)) {
                          echo "<tr><td colspan='9' class='text-center'>No hay registros disponibles</td></tr>";
                      } else {
                          // Get vigencia setting for tetanos records
                          $vigencia = 30; // Default value
                          try {
                              $configQuery = "SELECT e_vencimiento_tetanos FROM e_vencimiento LIMIT 1";
                              $configStmt = $conn->prepare($configQuery);
                              $configStmt->execute();
                              $row_config = $configStmt->fetch(PDO::FETCH_ASSOC);
                              if ($row_config && isset($row_config['e_vencimiento_tetanos'])) {
                                  $vigencia = intval($row_config['e_vencimiento_tetanos']);
                              }
                          } catch (PDOException $e) {
                          ...
                          foreach ($tetanossData as $row) {
                              ...
                              echo '        <button class="btn btn-success btn-sm register-new-tetanos-btn" 
                              ...
                              if ($row['in_tetanos_history'] == 1) {
                                  echo '        <button class="btn btn-warning btn-sm edit-tetanos" 
                                                  data-id="' . htmlspecialchars($row['tetanos_record_id'] ?? '') . '" 
                                                  data-tagid="' . htmlspecialchars($row['tagid'] ?? '') . '" 
                                                  data-vacuna="' . htmlspecialchars($row['ch_tetanos_producto'] ?? '') . '" 
                                                  data-dosis="' . htmlspecialchars($row['ch_tetanos_dosis'] ?? '') . '" 
                                                  data-costo="' . htmlspecialchars($row['ch_tetanos_costo'] ?? '') . '" 
                                                  data-fecha="' . htmlspecialchars($row['ch_tetanos_fecha'] ?? '') . '"
                                                  title="Editar Vacuna">
                                                  <i class="fas fa-edit"></i>
                                              </button>';
                                  echo '        <button class="btn btn-danger btn-sm delete-tetanos" 
                                                  data-id="' . htmlspecialchars($row['tetanos_record_id'] ?? '') . '"
                              ...
                              echo "<td>" . htmlspecialchars($row['ch_tetanos_fecha'] ?? '') . "</td>";
                              echo "<td>" . htmlspecialchars($row['nombre'] ?? 'N/A') . "</td>";
                              echo "<td>" . htmlspecialchars($row['tagid'] ?? '') . "</td>";

                              if ($row['in_tetanos_history'] == 1) {
                                  // Animal has vaccination history
                                  echo "<td>" . htmlspecialchars($row['ch_tetanos_producto'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['ch_tetanos_dosis'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['ch_tetanos_costo'] ?? 'N/A') . "</td>";

                                  // Calculate due date and determine status
                                  try {
                                      if (!empty($row['ch_tetanos_fecha'])) {
                                          $tetanosDate = new DateTime($row['ch_tetanos_fecha']);
                                          $dueDate = clone $tetanosDate;
                                      ...
                                      error_log("Date error: " . $e->getMessage() . " for date: " . $row['ch_tetanos_fecha']);
                      ...
                      error_log("Error in tetanos table: " . $e->getMessage());
...
<!-- Initialize DataTable for VH tetanos -->
<script>
$(document).ready(function() {
    $('#tetanosTable').DataTable({
...
$(document).ready(function() {
    ...
    // Note: editTetanosModal is created dynamically later, so no need to initialize here.

    // Handle new entry form submission
    $('#saveNewTetanos').click(function() {
        // Validate the form
        var form = document.getElementById('newTetanosForm');
        ...
        Swal.fire({
            ...
            text: `¿Desea registrar la dosis de tetanos ${formData.dosis} ml para el animal con Tag ID ${formData.tagid}?`,
            icon: 'question',
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        Swal.fire({
                            ...
                            text: 'El registro de tetanos ha sido guardado correctamente',
                            icon: 'success',
                            confirmButtonColor: '#28a745'
                        }).then(() => {
    ...
    $('.edit-tetanos').click(function() {
        ...
        // Edit Tetanos Modal dialog for editing
        var modalHtml = `
        <div class="modal fade" id="editTetanosModal" tabindex="-1" aria-labelledby="editTetanosModalLabel">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editTetanosModalLabel">
                            <i class="fas fa-weight me-2"></i>Editar Vacunacion Tetanos
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <form id="editTetanosForm">
                            ...
                            <div class="mb-4">                        
                                <div class="input-group">
                                    <span class="input-group-text">
                                        <i class="fa-solid fa-syringe"></i>
                                        <label for="edit_vacuna" class="form-label">Vacunas Tetanos</label>
                    ...
                    <div class="modal-footer btn-group">
                        ...
                        <button type="button" class="btn btn-success" id="saveEditTetanos">
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="vacunaSelectionModalLabel">
                            <i class="fas fa-list me-2"></i>Seleccionar Vacuna de Tetanos
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="list-group" id="vacunaList" style="max-height: 300px; overflow-y: auto;">
                                ...
                                try {
                                    ...
                                    $vacunas = $stmt_vacunas->fetchAll(PDO::FETCH_ASSOC);
                                    foreach ($vacunas as $vacuna_row) {
                                        echo '<button type="button" class="list-group-item list-group-item-action vacuna-option" data-value="' . htmlspecialchars($vacuna_row['cc_tetanos_vacuna']) . '">' . htmlspecialchars($vacuna_row['cc_tetanos_vacuna']) . '</button>';
                                    }
                                } catch (PDOException $e) {
                                    error_log("Error fetching vacunas: " . $e->getMessage());
        ...
        // Show the edit modal
        var editModal = new bootstrap.Modal(document.getElementById('editTetanosModal'));
        ...
        $('#saveEditTetanos').click(function() {
            var formData = {
                id: $('#edit_id').val(),
            ...
            Swal.fire({
                ...
                text: `¿Desea actualizar el registro de tetanos para el animal con Tag ID ${formData.tagid}?`,
                icon: 'question',
                showCancelButton: true,
                confirmButtonColor: '#28a745',
                cancelButtonColor: '#dc3545',
    ...
    $(document).on('click', '.register-new-tetanos-btn', function() { 
    ...
    <div class="card shadow-lg border-0">
        ...
        <div class="card-body p-4">
            ...
            <div class="chart-container">
                <canvas id="tetanosChart"></canvas>
            </div>
        </div>
    </div>
</div>

<!-- Professional CSS Styling for Tetanos Charts -->
<style>
...
<!-- Chart.js Script for Tetanos Line Chart -->
<script>
$(document).ready(function() {
    let allTetanosData = [];
    let tetanosChart = null;
    
    // Add loading state to chart container
    $('.chart-container').addClass('loading');
    
    // Fetch tetanos data and create the chart
    $.ajax({
        url: 'get_tetanos_data.php',
        type: 'GET',
        dataType: 'json',
        beforeSend: function() {
            $('.chart-container').addClass('loading');
        ...
        error: function(xhr, status, error) {
            $('.chart-container').removeClass('loading');
            console.error('Error fetching tetanos data:', error);
        }
    });
    ...
    function updateChart() {
        ...
        let filteredData = [...allTetanosData];
    ...
    function updateChartData(data) {
        if (tetanosChart) {
            tetanosChart.destroy();
        }
        createTetanosChart(data);
    }
    
    function createTetanosChart(data) {
        var ctx = document.getElementById('tetanosChart').getContext('2d');
...
<!-- Pie Chart Section -->
<div class="container mt-5 mb-5">
    <h4 class="text-center text-black mb-4">Estado de Vacunación Tetanos por Grupos</h4>
    <div class="row justify-content-center" id="groupChartsContainer">
        <!-- Group charts will be populated dynamically here -->
    </div>
</div>

<!-- Chart.js Script for Tetanos Status Pie Charts -->
<script>
$(document).ready(function() {
    const tetanosStatusUrl = 'get_tetanos_status_by_grupo.php';
                ...
                <div class="card shadow">
                    ...
                    <div class="card-body">
                        <div class="chart-container" style="position: relative; height:40vh; width:100%">
                            <canvas id="tetanosPie${safeGrupo}"></canvas>
                            <div id="tetanosPie${safeGrupo}Message" class="text-center small text-muted mt-2"></div>
                        </div>
    ...
    $.ajax({
        url: tetanosStatusUrl,
        type: 'GET',
        dataType: 'json',
        success: function(data) {
            if (data.error) {
                console.error('Error fetching tetanos status data:', data.error);
            ...
            grupos.forEach(grupo => {
                ...
                const canvasId = `tetanosPie${safeGrupo}`;
                const messageId = `tetanosPie${safeGrupo}Message`;
        ...
        error: function(xhr, status, error) {
            console.error('AJAX error fetching tetanos status data:', error);
