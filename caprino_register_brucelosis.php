...
<title>Caprino - Registro de Vacunación Brucelosis</title>
<!-- Link to the Favicon -->
...
<div class="container text-center">
  ...
  REGISTROS VACUNACION BRUCELOSIS
  </h3>
                        ...
                        <div class="input-group">
                            <span class="input-group-text">
                                ...
                                <select class="form-select" id="new_vacuna" name="vacuna" required>
                                    ...
                                    <?php
                                    try {
                                        $sql_vacunas = "SELECT DISTINCT cc_brucelosis_vacuna FROM cc_brucelosis ORDER BY cc_brucelosis_vacuna ASC";
                                        ...
                                        foreach ($vacunas as $vacuna_row) {
                                            echo '<option value="' . htmlspecialchars($vacuna_row['cc_brucelosis_vacuna']) . '">' . htmlspecialchars($vacuna_row['cc_brucelosis_vacuna']) . '</option>';
                                        }
  ...
  <!-- DataTable for ch_brucelosis records -->
              ...
              <tbody>
                  ...
                  try {
                      $brucelosisQuery = "SELECT 
                                          ...
                                          v.tagid,
                                          v.nombre,
                                          va.ch_brucelosis_fecha,
                                          va.id AS brucelosis_record_id, -- Alias brucelosis record ID
                                          va.ch_brucelosis_producto,
                                          va.ch_brucelosis_dosis,
                                          va.ch_brucelosis_costo,
                                          -- Flag to easily check if there's a history record
                                          CASE WHEN va.ch_brucelosis_tagid IS NOT NULL THEN 1 ELSE 0 END AS in_brucelosis_history
                                      FROM 
                                          caprino v
                                      LEFT JOIN 
                                          -- Join with all brucelosis records per tagid
                                          ch_brucelosis va ON v.tagid = va.ch_brucelosis_tagid 
                                      ORDER BY 
                                          va.ch_brucelosis_fecha DESC, v.tagid ASC;";
                      ...
                      if (empty($brucelosisData)) {
                          ...
                          foreach ($brucelosisData as $row) {
                              ...
                              echo '        <button class="btn btn-success btn-sm register-new-brucelosis-btn" 
                              ...
                              if ($row['in_brucelosis_history'] == 1) {
                                  echo '        <button class="btn btn-warning btn-sm edit-brucelosis" 
                                                  ...
                                                  data-vacuna="' . htmlspecialchars($row['ch_brucelosis_producto'] ?? '') . '" 
                                                  data-dosis="' . htmlspecialchars($row['ch_brucelosis_dosis'] ?? '') . '" 
                                                  data-costo="' . htmlspecialchars($row['ch_brucelosis_costo'] ?? '') . '" 
                                                  data-fecha="' . htmlspecialchars($row['ch_brucelosis_fecha'] ?? '') . '"
                                                  title="Editar Vacuna">
                              ...
                              echo "<td>" . htmlspecialchars($row['ch_brucelosis_fecha'] ?? '') . "</td>";
                              echo "<td>" . htmlspecialchars($row['nombre'] ?? 'N/A') . "</td>";
                              echo "<td>" . htmlspecialchars($row['tagid'] ?? '') . "</td>";

                              if ($row['in_brucelosis_history'] == 1) {
                                  // Animal has vaccination history
                                  echo "<td>" . htmlspecialchars($row['ch_brucelosis_producto'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['ch_brucelosis_dosis'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['ch_brucelosis_costo'] ?? 'N/A') . "</td>";

                                  // Calculate due date and determine status
                                  try {
                                      if (!empty($row['ch_brucelosis_fecha'])) {
                                          $brucelosisDate = new DateTime($row['ch_brucelosis_fecha']);
                                      ...
                                      error_log("Date error: " . $e->getMessage() . " for date: " . $row['ch_brucelosis_fecha']);
                        ...
                        <div class="mb-3">
                            ...
                            <div class="list-group" id="vacunaList" style="max-height: 300px; overflow-y: auto;">
                                ...
                                try {
                                    ...
                                    $vacunas = $stmt_vacunas->fetchAll(PDO::FETCH_ASSOC);
                                    foreach ($vacunas as $vacuna_row) {
                                        echo '<button type="button" class="list-group-item list-group-item-action vacuna-option" data-value="' . htmlspecialchars($vacuna_row['cc_brucelosis_vacuna']) . '">' . htmlspecialchars($vacuna_row['cc_brucelosis_vacuna']) . '</button>';
                                    }
                                } catch (PDOException $e) {
                                    error_log("Error fetching vacunas: " . $e->getMessage());
