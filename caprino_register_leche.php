...
try {
    ...
    $lecheQuery = "SELECT 
                    ...
                    AND ch_leche_fecha_fin IS NOT NULL 
                    AND ch_leche_peso IS NOT NULL 
                    AND ch_leche_precio IS NOT NULL
    ...
    foreach ($lecheData as $leche) {
        $startDate = new DateTime($leche['ch_leche_fecha_inicio']);
        $endDate = new DateTime($leche['ch_leche_fecha_fin']);
        
        // Calculate total production value
        $totalDays = $endDate->diff($startDate)->days + 1;
        $totalValue = $leche['ch_leche_peso'] * $leche['ch_leche_precio'] * $totalDays;
...
<title>Caprino Registro Leche</title>
<!-- Link to the Favicon -->
...
<div class="container text-center">
  ...
  <!-- DataTable for ch_leche records -->
                  ...
                  try {
                      // Query to get all Female Animals and ALL their milk records (if any)
                        $milkQuery = "
                            SELECT
                                ...
                                l.ch_leche_fecha_inicio,
                                l.ch_leche_tagid,         -- Matches caprino_tagid if milk record exists
                                l.ch_leche_peso,
                                l.ch_leche_precio,
                                -- Calculate total_value only if l.id is not null
                                CASE WHEN l.id IS NOT NULL THEN CAST((l.ch_leche_peso * l.ch_leche_precio) AS DECIMAL(10,2)) ELSE NULL END as total_value
                            FROM
                                caprino b
                            LEFT JOIN
                                ch_leche l ON b.tagid = l.ch_leche_tagid -- Join ALL matching milk records
                            WHERE
                      ...
                      if (empty($milksData)) {
                          ...
                          foreach ($milksData as $row) {
                              ...
                              $lecheFecha = $row['ch_leche_fecha_inicio'] ?? null;
                              ...
                              if ($hasLeche) {
                                  ...
                                  echo '        <button class="btn btn-warning btn-sm edit-peso" 
                                                  ...
                                                  data-tagid="'.htmlspecialchars($row['ch_leche_tagid'] ?? '').'" 
                                                  data-peso="'.htmlspecialchars($row['ch_leche_peso'] ?? '').'" 
                                                  data-precio="'.htmlspecialchars($row['ch_leche_precio'] ?? '').'" 
                                  ...
                                  echo '        <button class="btn btn-danger btn-sm delete-peso" 
                                                  data-id="'.htmlspecialchars($row['leche_id'] ?? '').'" 
                                                  data-tagid="'.htmlspecialchars($row['ch_leche_tagid'] ?? '').'" -- Pass tagid for context
                              ...
                              echo "<td>" . htmlspecialchars($row['caprino_tagid'] ?? 'N/A') . "</td>"; // Use caprino_tagid for consistency
                              // Column 5: Leche (kg)
                              echo "<td>" . ($hasLeche ? htmlspecialchars($row['ch_leche_peso'] ?? '') : 'N/A') . "</td>";
                              // Column 6: Precio ($/kg)
                              echo "<td>" . ($hasLeche ? htmlspecialchars($row['ch_leche_precio'] ?? '') : 'N/A') . "</td>";
