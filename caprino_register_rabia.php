...
<title>Caprino - Registro de Vacunación Rabia</title>
<!-- Link to the Favicon -->
...
<div class="container text-center">
  ...
  REGISTROS VACUNACION RABIA
  </h3>
  
  <!-- New Rabia Entry Modal -->
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="newEntryModalLabel">
                    <i class="fas fa-plus-circle me-2"></i>Nueva Vacunacion Rabia
                </h5>
                    ...
                    <div class="mb-4">
                        <div class="input-group">
                            <span class="input-group-text">
                                ...
                                <label for="new_vacuna" class="form-label">Vacunas Rabia</label>
                                <select class="form-select" id="new_vacuna" name="vacuna" required>
                                    <option value="">Vacunas</option>
                                    <?php
                                    try {
                                        $sql_vacunas = "SELECT DISTINCT cc_rabia_vacuna FROM cc_rabia ORDER BY cc_rabia_vacuna ASC";
                                        ...
                                        foreach ($vacunas as $vacuna_row) {
                                            echo '<option value="' . htmlspecialchars($vacuna_row['cc_rabia_vacuna']) . '">' . htmlspecialchars($vacuna_row['cc_rabia_vacuna']) . '</option>';
                                        }
  ...
  <!-- DataTable for ch_rabia records -->
              ...
              <tbody>
                  ...
                  try {
                      $rabiaQuery = "SELECT 
                                          ...
                                          v.nombre,
                                          va.ch_rabia_fecha,
                                          va.id AS rabia_record_id, -- Alias rabia record ID
                                          va.ch_rabia_producto,
                                          va.ch_rabia_dosis,
                                          va.ch_rabia_costo,
                                          -- Flag to easily check if there's a history record
                                          CASE WHEN va.ch_rabia_tagid IS NOT NULL THEN 1 ELSE 0 END AS in_rabia_history
                                      FROM 
                                          caprino v
                                      LEFT JOIN 
                                          -- Join with all rabia records per tagid
                                          ch_rabia va ON v.tagid = va.ch_rabia_tagid 
                                      ORDER BY 
                                          va.ch_rabia_fecha DESC, v.tagid ASC;";
                      ...
                      if (empty($rabiasData)) {
                          ...
                          // Get vigencia setting for rabia records
                          $vigencia = 30; // Default value
                          try {
                              $configQuery = "SELECT c_vencimiento_rabia FROM c_vencimiento LIMIT 1";
                              $configStmt = $conn->prepare($configQuery);
                              $configStmt->execute();
                              $row_config = $configStmt->fetch(PDO::FETCH_ASSOC);
                              if ($row_config && isset($row_config['c_vencimiento_rabia'])) {
                                  $vigencia = intval($row_config['c_vencimiento_rabia']);
                              }
                          } catch (PDOException $e) {
                          ...
                          foreach ($rabiasData as $row) {
                              ...
                              echo '        <button class="btn btn-success btn-sm register-new-rabia-btn" 
                              ...
                              if ($row['in_rabia_history'] == 1) {
                                  echo '        <button class="btn btn-warning btn-sm edit-rabia" 
                                                  data-id="' . htmlspecialchars($row['rabia_record_id'] ?? '') . '" 
                                                  data-tagid="' . htmlspecialchars($row['tagid'] ?? '') . '" 
                                                  data-vacuna="' . htmlspecialchars($row['ch_rabia_producto'] ?? '') . '" 
                                                  data-dosis="' . htmlspecialchars($row['ch_rabia_dosis'] ?? '') . '" 
                                                  data-costo="' . htmlspecialchars($row['ch_rabia_costo'] ?? '') . '" 
                                                  data-fecha="' . htmlspecialchars($row['ch_rabia_fecha'] ?? '') . '"
                                                  title="Editar Vacuna">
                              ...
                              echo "<td>" . htmlspecialchars($row['ch_rabia_fecha'] ?? '') . "</td>";
                              echo "<td>" . htmlspecialchars($row['nombre'] ?? 'N/A') . "</td>";
                              echo "<td>" . htmlspecialchars($row['tagid'] ?? '') . "</td>";

                              if ($row['in_rabia_history'] == 1) {
                                  // Animal has vaccination history
                                  echo "<td>" . htmlspecialchars($row['ch_rabia_producto'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['ch_rabia_dosis'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['ch_rabia_costo'] ?? 'N/A') . "</td>";

                                  // Calculate due date and determine status
                                  try {
                                      if (!empty($row['ch_rabia_fecha'])) {
                                          $rabiaDate = new DateTime($row['ch_rabia_fecha']);
                                          $dueDate = clone $rabiaDate;
                                      ...
                                      error_log("Date error: " . $e->getMessage() . " for date: " . $row['ch_rabia_fecha']);
                      ...
                      error_log("Error in rabia table: " . $e->getMessage());
...
$(document).ready(function() {
    ...
    $('#saveNewRabia').click(function() {
        ...
        Swal.fire({
            ...
            text: `¿Desea registrar la dosis de rabia ${formData.dosis} ml para el animal con Tag ID ${formData.tagid}?`,
            icon: 'question',
    ...
    $('.edit-rabia').click(function() {
        ...
        // Edit Rabia Modal dialog for editing
        var modalHtml = `
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editRabiaModalLabel">
                            <i class="fas fa-weight me-2"></i>Editar Vacunacion Rabia
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <form id="editRabiaForm">
                            ...
                            <div class="mb-4">                        
                                <div class="input-group">
                                    <span class="input-group-text">
                                        <i class="fa-solid fa-syringe"></i>
                                        <label for="edit_vacuna" class="form-label">Vacunas Rabia</label>
                        ...
                        <div class="mb-3">
                            ...
                            <div class="list-group" id="vacunaList" style="max-height: 300px; overflow-y: auto;">
                                ...
                                try {
                                    ...
                                    $vacunas = $stmt_vacunas->fetchAll(PDO::FETCH_ASSOC);
                                    foreach ($vacunas as $vacuna_row) {
                                        echo '<button type="button" class="list-group-item list-group-item-action vacuna-option" data-value="' . htmlspecialchars($vacuna_row['cc_rabia_vacuna']) . '">' . htmlspecialchars($vacuna_row['cc_rabia_vacuna']) . '</button>';
                                    }
                                } catch (PDOException $e) {
                                    error_log("Error fetching vacunas: " . $e->getMessage());
        ...
        $('#saveEditRabia').click(function() {
            ...
            Swal.fire({
                ...
                text: `¿Desea actualizar el registro de rabia para el animal con Tag ID ${formData.tagid}?`,
                icon: 'question',
                showCancelButton: true,
                confirmButtonColor: '#28a745',
                cancelButtonColor: '#dc3545',
...
$(document).ready(function() {
    ...
    function createRabiaChart(data) {
        var ctx = document.getElementById('rabiaChart').getContext('2d');
...
<!-- Pie Chart Section -->
<div class="container mt-5 mb-5">
    <h4 class="text-center text-black mb-4">Estado de Vacunación Rabia por Grupos</h4>
    <div class="row justify-content-center" id="groupChartsContainer">
        <!-- Group charts will be populated dynamically here -->
    </div>
</div>

<!-- Chart.js Script for Rabia Status Pie Charts -->
<script>
$(document).ready(function() {
    const rabiaStatusUrl = 'get_rabia_status_by_grupo.php';
                ...
                <div class="card shadow">
                    ...
                    <div class="card-body">
                        <div class="chart-container" style="position: relative; height:40vh; width:100%">
                            <canvas id="rabiaPie${safeGrupo}"></canvas>
                            <div id="rabiaPie${safeGrupo}Message" class="text-center small text-muted mt-2"></div>
                        </div>
    ...
    $.ajax({
        ...
        success: function(data) {
            if (data.error) {
                console.error('Error fetching rabia status data:', data.error);
            ...
            grupos.forEach(grupo => {
                ...
                const canvasId = `rabiaPie${safeGrupo}`;
                const messageId = `rabiaPie${safeGrupo}Message`;
        ...
        error: function(xhr, status, error) {
            console.error('AJAX error fetching rabia status data:', error);
