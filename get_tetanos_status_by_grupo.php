...
try {
    ...
    $sql_groups = "SELECT DISTINCT grupo FROM caprino WHERE grupo IS NOT NULL AND grupo != '' ORDER BY grupo";
    ...
    $sql_counts = "
        SELECT
            ...
            COUNT(DISTINCT a.ch_tetanos_tagid) AS vaccinated_animals
        FROM caprino v
        LEFT JOIN ch_tetanos a ON v.tagid = a.ch_tetanos_tagid
    ...
    $sql_list = "
        ...
        LEFT JOIN ch_tetanos a ON v.tagid = a.ch_tetanos_tagid
        WHERE v.grupo IS NOT NULL AND v.grupo != ''
          AND a.ch_tetanos_tagid IS NULL
    ...
    error_log("Error fetching Caprino status by grupo: " . $e->getMessage());
