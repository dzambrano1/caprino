...
try {
    // Fetch all purchases ordered by date from caprino table
...
<title>Caprino Register Compras</title>
<!-- Link to the Favicon -->
...
<div class="container text-center">
  ...
  <div class="container mt-3 mb-4 text-center">
    <button type="button" class="btn btn-outline-success" data-bs-toggle="modal" data-bs-target="#newEntryModal">
...
document.getElementById('updateModal').addEventListener('shown.bs.modal', function () {
  ...
  <!-- DataTable for ch_compra records -->
      ...
      <div class="table-responsive">
          <table id="compraTable" class="table table-striped table-bordered">
              ...
              <tbody>
                  <?php
                  try {
                      // Query to get purchase data directly from caprino table
                      $compraQuery = "SELECT 
                                        id, 
                  ...
                  foreach ($compraData as $row) {
                      ...
                      // Render row using caprino table columns
                      echo "<tr>";
                      ...
                      echo "<td class='text-center'>" . htmlspecialchars($row['animal_nombre'] ?? 'N/A') . "</td>"; // caprino.nombre                      
                      echo "<td class='text-center'>" . htmlspecialchars($row['fecha_compra_formatted'] ?? 'N/A') . "</td>"; // Use formatted fecha_compra
                      echo "<td class='text-center'>" . htmlspecialchars($row['tagid'] ?? '') . "</td>"; // caprino.tagid
                      echo "<td class='text-center'>" . htmlspecialchars($row['precio_compra'] ?? '0.00') . "</td>"; // caprino.precio_compra
                      echo "<td class='text-center'>" . htmlspecialchars($row['peso_compra'] ?? '0.00') . "</td>"; // caprino.peso_compra
                      echo "</tr>";
                  }
                  ?>
              </tbody>
...
$(document).ready(function() {
    ...
    $('.edit-compra').click(function() {
        ...
        $.ajax({
            url: 'caprino_get_details.php', // Assuming this endpoint exists
            type: 'GET',
            data: { tagid: tagid },
            dataType: 'json',
