...
<title>caprino Register Aborto</title>
<!-- Link to the Favicon -->
...
<div class="container nav-icons-container">
    ...
    <div class="icon-button-container">
        <button onclick="window.location.href='./caprino_feria.php'" class="icon-button">
...
<div class="container scroll-icons-container">
    <button class="btn btn-outline-secondary mb-3" type="button" data-bs-toggle="collapse" data-bs-target="#section-inventario-poblacion-caprino" data-tooltip="Poblacion">
        <img src="./images/vaca.png" alt="Alimentacion" class="nav-icon">
    </button>

    <button class="btn btn-outline-secondary mb-3" type="button" data-bs-toggle="collapse" data-bs-target="#section-inventario-produccion-caprino" data-tooltip="Produccion">
        <img src="./images/Bascula.png" alt="Produccion" class="nav-icon">
    </button>

    <button class="btn btn-outline-secondary mb-3" type="button" data-bs-toggle="collapse" data-bs-target="#section-inventario-alimentacion-caprino" data-tooltip="Alimentacion">
        <img src="./images/bolso.png" alt="Alimentacion" class="nav-icon">
    </button>

    <button class="btn btn-outline-secondary mb-3" type="button" data-bs-toggle="collapse" data-bs-target="#section-inventario-salud-caprino" data-tooltip="Salud">
        <img src="./images/vacunacion.png" alt="Salud" class="nav-icon">
    </button>
       
    <button class="btn btn-outline-secondary mb-3" type="button" data-bs-toggle="collapse" data-bs-target="#section-inventario-reproduccion-caprino" data-tooltip="Reproducion">
        <img src="./images/matriz.png" alt="Razas" class="nav-icon">
    </button>

    <button class="btn btn-outline-secondary mb-3" type="button" data-bs-toggle="collapse" data-bs-target="#section-inventario-otros-caprino" data-tooltip="Otros">
        <img src="./images/compra.png" alt="Razas" class="nav-icon">
    </button>
</div>

<!-- Add back button before the header container -->
<a href="./inventario_caprino.php" class="back-btn">
...
<div class="container text-center">
  ...
  <!-- DataTable for ch_aborto records -->
  
  <div class="container table-section" style="display: block;">
      <div class="table-responsive">
          <table id="ch_abortoTable" class="table table-striped table-bordered">
              <thead>
                  <tr>
              ...
              <tbody>
                  <?php
                  try {
                      // Query to get all ch_aborto records with animal name
                      $abortoQuery = "SELECT c.*, v.nombre as animal_nombre
                                FROM ch_aborto c 
                                LEFT JOIN caprino v ON c.ch_aborto_tagid = v.tagid 
                                ORDER BY c.ch_aborto_fecha DESC";
                      ...
                      if (empty($abortoData)) {
                          ...
                          foreach ($abortoData as $row) {
                              ...
                              echo "<td class='text-center'>" . htmlspecialchars($row['ch_aborto_fecha'] ?? '') . "</td>";                        
                              echo "<td class='text-center'>" . htmlspecialchars($row['animal_nombre'] ?? 'N/A') . "</td>";
                              echo "<td class='text-center'>" . htmlspecialchars($row['ch_aborto_tagid'] ?? '') . "</td>";
                              echo "<td class='text-center'>" . htmlspecialchars($row['ch_aborto_causa'] ?? '') . "</td>";
                              
                              // Calculate due date and determine status
                              try {
                                  $abortoDate = new DateTime($row['ch_aborto_fecha']);
                                  ...
                                  error_log("Date error: " . $e->getMessage() . " for date: " . $row['ch_aborto_fecha']);
                              ...
                              echo '<td class="text-center">
                                  <div class="btn-group" role="group">
                                      <button class="btn btn-warning btn-sm edit-aborto" 
                                          ...
                                          data-tagid="' . htmlspecialchars($row['ch_aborto_tagid'] ?? '') . '"
                                          data-fecha="' . htmlspecialchars($row['ch_aborto_fecha'] ?? '') . '"
                                          data-causa="' . htmlspecialchars($row['ch_aborto_causa'] ?? '') . '">
...
$(document).ready(function() {
    ...
    function populateAnimalFilter(data) {
        ...
        data.forEach(item => {
            if (item.ch_aborto_tagid && !uniqueTagIds.has(item.ch_aborto_tagid)) {
                uniqueTagIds.add(item.ch_aborto_tagid);
                animals.push({
                    tagid: item.ch_aborto_tagid,
    ...
    function updateChart() {
        ...
        if (selectedAnimal !== 'all') {
            filteredData = filteredData.filter(item => item.ch_aborto_tagid === selectedAnimal);
        }
        
        // Sort data by date
        filteredData.sort((a, b) => new Date(a.ch_aborto_fecha) - new Date(b.ch_aborto_fecha));
    ...
    function createAbortoChart(data) {
        ...
        var labels = data.map(function(item) {
            ...
            var parts = item.ch_aborto_fecha.split('-');
            if (parts.length === 3) {
                return parts[2] + '/' + parts[1] + '/' + parts[0];
            }
            return item.ch_aborto_fecha;
        });
        
        var causa = data.map(function(item) {
            return item.ch_aborto_causa;
        });
