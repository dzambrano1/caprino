...
try {
    ...
    $sql = "SELECT 
                ...
                SUM(ch_peso_animal * ch_peso_precio) as total_revenue,
                SUM(ch_peso_animal) as total_quantity,
                AVG(ch_peso_precio) as avg_price,
                COUNT(*) as record_count
            FROM ch_peso 
            WHERE ch_peso_fecha IS NOT NULL 
            AND ch_peso_animal > 0 
            AND ch_peso_precio > 0
            GROUP BY DATE_FORMAT(ch_peso_fecha, '%Y-%m')
            ORDER BY DATE_FORMAT(ch_peso_fecha, '%Y-%m') ASC";
