...
try {
    ...
    $sql = "SELECT 
                DATE_FORMAT(ch_parto_fecha, '%Y-%m') as month,
                COUNT(*) as birth_count,
                AVG(ch_parto_numero) as avg_birth_number,
                MAX(ch_parto_numero) as max_birth_number,
                COUNT(*) as record_count
            FROM ch_parto 
            WHERE ch_parto_fecha IS NOT NULL 
            AND ch_parto_numero > 0
            GROUP BY DATE_FORMAT(ch_parto_fecha, '%Y-%m')
            ORDER BY DATE_FORMAT(ch_parto_fecha, '%Y-%m') ASC";
