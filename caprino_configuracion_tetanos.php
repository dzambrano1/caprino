...
<title>Caprino Configuracion Tetanos</title>
<!-- Link to the Favicon -->
...
<div class="container text-center">
  ...
  CONFIGURACION VACUNAS TETANOS
  </h3>
</div> 
<!-- New Entry Modal Configuracion Tetanos -->

<!-- Add New Vacuna Tetanos Button -->
<div class="container my-3 text-center">
  <button type="button" class="btn btn-success text-center" data-bs-toggle="modal" data-bs-target="#newEntryModal">
    <i class="fas fa-plus-circle me-2"></i>Nueva Vacuna Tetanos
  </button>
</div>
...
<div class="modal fade" id="newEntryModal" tabindex="-1" aria-labelledby="newEntryModalLabel">
      ...
      <div class="modal-content">
          <div class="modal-header">
              <h5 class="modal-title" id="newEntryModalLabel">
                  <i class="fas fa-plus-circle me-2"></i>Configurar Nueva Vacuna Tetanos
              </h5>
  ...
  <!-- DataTable for vh_tetanos records -->
...
<div class="container table-section" style="display: block;">
      <div class="table-responsive">
          <table id="tetanosTable" class="table table-striped table-bordered">
              ...
              <tbody>
                  <?php
                      $tetanosQuery = "SELECT * FROM cc_tetanos";
                      ...
                      if (empty($tetanossData)) {
                          ...
                          foreach ($tetanossData as $row) {
                              ...
                              echo '        <button class="btn btn-warning btn-sm edit-tetanos" 
                                              ...
                                              data-vacuna="' . htmlspecialchars($row['cc_tetanos_vacuna'] ?? '') . '" 
                                              data-dosis="' . htmlspecialchars($row['cc_tetanos_dosis'] ?? '') . '" 
                                              data-costo="' . htmlspecialchars($row['cc_tetanos_costo'] ?? '') . '" 
                                              data-vigencia="' . htmlspecialchars($row['cc_tetanos_vigencia'] ?? '') . '"
                                              title="Editar Configuracion Vacuna Tetanos">
                                              <i class="fas fa-edit"></i>
                                          </button>';
                              echo '        <button class="btn btn-danger btn-sm delete-tetanos" 
                                              data-id="' . htmlspecialchars($row['id'] ?? '') . '"
                                              title="Eliminar Configuracion Vacuna Tetanos">
                              ...
                              // Column 1: Vacuna
                              echo "<td>" . htmlspecialchars($row['cc_tetanos_vacuna'] ?? '') . "</td>";
                              
                              // Column 2: Dosis
                              echo "<td>" . htmlspecialchars($row['cc_tetanos_dosis'] ?? 'N/A') . "</td>";
                              
                              // Column 3: Costo
                              echo "<td>" . htmlspecialchars($row['cc_tetanos_costo'] ?? 'N/A') . "</td>";
                              
                              // Column 4: Vigencia
                              echo "<td>" . htmlspecialchars($row['cc_tetanos_vigencia'] ?? 'N/A') . "</td>";
...
$(document).ready(function() {
    ...
    $('#saveNewTetanos').click(function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: 'process_configuracion_tetanos.php',
                    type: 'POST',
                    data: {
                        action: 'insert',
    ...
    $('.edit-tetanos').click(function() {
        ...
        // Edit Configuracion Tetanos Modal dialog for editing
        var modalHtml = `
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editTetanosModalLabel">
                            <i class="fas fa-weight me-2"></i>Editar Vacuna Tetanos
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
        ...
        $('#saveEditTetanos').click(function() {
            ...
            var form = document.getElementById('editTetanosForm');
            if (!form.checkValidity()) {
                form.reportValidity();
                return;
            }
            ...
            Swal.fire({
                ...
                text: `¿Desea actualizar la configuracion de tetanos?`,
                icon: 'question',
                showCancelButton: true,
                confirmButtonColor: '#28a745',
                cancelButtonColor: '#dc3545',
    ...
    $('.delete-tetanos').click(function() {
        ...
        Swal.fire({
            ...
            text: `¿Está seguro de que desea eliminar la configuracion de tetanos? Esta acción no se puede deshacer.`,
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#dc3545',
