...
<title>Caprino - Registro de Vacunación Clostridiosis</title>
<!-- Link to the Favicon -->
...
<div class="container text-center">
  ...
  REGISTROS DE CLOSTRIDIOSIS
  </h3>
  
  <!-- New Clostridiosis Entry Modal -->
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="newEntryModalLabel">
                    <i class="fas fa-plus-circle me-2"></i>Nueva Vacunacion Clostridiosis
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form id="newClostridiosisForm">
                <div class="mb-4">
                    ...
                    <div class="mb-4">                        
                        <div class="input-group">
                            <span class="input-group-text">
                                ...
                                <label for="new_vacuna" class="form-label">Vacunas Clostridiosis</label>
                                <select class="form-select" id="new_vacuna" name="vacuna" required>
                                    <option value="">Vacunas</option>
                                    <?php
                                    try {
                                        $sql_vacunas = "SELECT DISTINCT cc_clostridiosis_vacuna FROM cc_clostridiosis ORDER BY cc_clostridiosis_vacuna ASC";
                                        ...
                                        foreach ($vacunas as $vacuna_row) {
                                            echo '<option value="' . htmlspecialchars($vacuna_row['cc_clostridiosis_vacuna']) . '">' . htmlspecialchars($vacuna_row['cc_clostridiosis_vacuna']) . '</option>';
                                        }
            ...
            <div class="modal-footer btn-group">
                ...
                <button type="button" class="btn btn-success" id="saveNewClostridiosis">
  ...
  <!-- DataTable for ch_clostridiosis records -->
  
  <div class="container table-section" style="display: block;">
      <div class="table-responsive">
          <table id="clostridiosisTable" class="table table-striped table-bordered">
              <thead>
                  <tr>
              ...
              <tbody>
                  <?php
                  try {
                      $clostridiosisQuery = "SELECT 
                                          v.id AS caprino_id, -- Alias caprino ID
                                          v.tagid,
                                          v.nombre,
                                          va.ch_clostridiosis_fecha,
                                          va.id AS clostridiosis_record_id, -- Alias clostridiosis record ID
                                          va.ch_clostridiosis_producto,
                                          va.ch_clostridiosis_dosis,
                                          va.ch_clostridiosis_costo,
                                          -- Flag to easily check if there's a history record
                                          CASE WHEN va.ch_clostridiosis_tagid IS NOT NULL THEN 1 ELSE 0 END AS in_clostridiosis_history
                                      FROM 
                                          caprino v
                                      LEFT JOIN 
                                          -- Join with all clostridiosis records per tagid
                                          ch_clostridiosis va ON v.tagid = va.ch_clostridiosis_tagid 
                                      ORDER BY 
                                          va.ch_clostridiosis_fecha DESC, v.tagid ASC;";

                      $stmt = $conn->prepare($clostridiosisQuery);
                      $stmt->execute();
                      $clostridiosissData = $stmt->fetchAll(PDO::FETCH_ASSOC);

                      if (empty($clostridiosissData)) {
                          echo "<tr><td colspan='9' class='text-center'>No hay registros disponibles</td></tr>";
                      } else {
                          // Get vigencia setting for clostridiosis records
                          $vigencia = 30; // Default value
                          try {
                              $configQuery = "SELECT v_vencimiento_clostridiosis FROM v_vencimiento LIMIT 1";
                              $configStmt = $conn->prepare($configQuery);
                              $configStmt->execute();
                              $row_config = $configStmt->fetch(PDO::FETCH_ASSOC);
                              if ($row_config && isset($row_config['v_vencimiento_clostridiosis'])) {
                                  $vigencia = intval($row_config['v_vencimiento_clostridiosis']);
                              }
                          } catch (PDOException $e) {
                          ...
                          foreach ($clostridiosissData as $row) {
                              echo "<tr>";
                              ...
                              echo '        <button class="btn btn-success btn-sm register-new-clostridiosis-btn" 
                              ...
                              if ($row['in_clostridiosis_history'] == 1) {
                                  echo '        <button class="btn btn-warning btn-sm edit-clostridiosis" 
                                                  data-id="' . htmlspecialchars($row['clostridiosis_record_id'] ?? '') . '" 
                                                  data-tagid="' . htmlspecialchars($row['tagid'] ?? '') . '" 
                                                  data-vacuna="' . htmlspecialchars($row['ch_clostridiosis_producto'] ?? '') . '" 
                                                  data-dosis="' . htmlspecialchars($row['ch_clostridiosis_dosis'] ?? '') . '" 
                                                  data-costo="' . htmlspecialchars($row['ch_clostridiosis_costo'] ?? '') . '" 
                                                  data-fecha="' . htmlspecialchars($row['ch_clostridiosis_fecha'] ?? '') . '"
                                                  title="Editar Vacuna">
                                                  <i class="fas fa-edit"></i>
                                              </button>';
                                  echo '        <button class="btn btn-danger btn-sm delete-clostridiosis" 
                                                  data-id="' . htmlspecialchars($row['clostridiosis_record_id'] ?? '') . '"
                              ...
                              echo "<td>" . htmlspecialchars($row['ch_clostridiosis_fecha'] ?? '') . "</td>";
                              echo "<td>" . htmlspecialchars($row['nombre'] ?? 'N/A') . "</td>";
                              echo "<td>" . htmlspecialchars($row['tagid'] ?? '') . "</td>";

                              if ($row['in_clostridiosis_history'] == 1) {
                                  // Animal has vaccination history
                                  echo "<td>" . htmlspecialchars($row['ch_clostridiosis_producto'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['ch_clostridiosis_dosis'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['ch_clostridiosis_costo'] ?? 'N/A') . "</td>";

                                  // Calculate due date and determine status
                                  try {
                                      if (!empty($row['ch_clostridiosis_fecha'])) {
                                          $clostridiosisDate = new DateTime($row['ch_clostridiosis_fecha']);
                                          $dueDate = clone $clostridiosisDate;
                                      ...
                                      error_log("Date error: " . $e->getMessage() . " for date: " . $row['ch_clostridiosis_fecha']);
                      ...
                      error_log("Error in clostridiosis table: " . $e->getMessage());
...
<!-- Initialize DataTable for VH clostridiosis -->
<script>
$(document).ready(function() {
    $('#clostridiosisTable').DataTable({
...
$(document).ready(function() {
    ...
    // Note: editClostridiosisModal is created dynamically later, so no need to initialize here.

    // Handle new entry form submission
    $('#saveNewClostridiosis').click(function() {
        // Validate the form
        var form = document.getElementById('newClostridiosisForm');
        ...
        Swal.fire({
            ...
            text: `¿Desea registrar la dosis de clostridiosis ${formData.dosis} ml para el animal con Tag ID ${formData.tagid}?`,
            icon: 'question',
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        Swal.fire({
                            ...
                            text: 'El registro de clostridiosis ha sido guardado correctamente',
                            icon: 'success',
                            confirmButtonColor: '#28a745'
                        }).then(() => {
    ...
    $('.edit-clostridiosis').click(function() {
        var id = $(this).data('id');
        ...
        // Edit Clostridiosis Modal dialog for editing
        var modalHtml = `
        <div class="modal fade" id="editClostridiosisModal" tabindex="-1" aria-labelledby="editClostridiosisModalLabel">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editClostridiosisModalLabel">
                            <i class="fas fa-weight me-2"></i>Editar Vacunacion Clostridiosis
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <form id="editClostridiosisForm">
                            ...
                            <div class="mb-4">                        
                                <div class="input-group">
                                    <span class="input-group-text">
                                        <i class="fa-solid fa-syringe"></i>
                                        <label for="edit_vacuna" class="form-label">Vacunas Clostridiosis</label>
                    ...
                    <div class="modal-footer btn-group">
                        ...
                        <button type="button" class="btn btn-success" id="saveEditClostridiosis">
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="vacunaSelectionModalLabel">
                            <i class="fas fa-list me-2"></i>Seleccionar Vacuna de Clostridiosis
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="list-group" id="vacunaList" style="max-height: 300px; overflow-y: auto;">
                                ...
                                try {
                                    ...
                                    $vacunas = $stmt_vacunas->fetchAll(PDO::FETCH_ASSOC);
                                    foreach ($vacunas as $vacuna_row) {
                                        echo '<button type="button" class="list-group-item list-group-item-action vacuna-option" data-value="' . htmlspecialchars($vacuna_row['cc_clostridiosis_vacuna']) . '">' . htmlspecialchars($vacuna_row['cc_clostridiosis_vacuna']) . '</button>';
                                    }
                                } catch (PDOException $e) {
                                    error_log("Error fetching vacunas: " . $e->getMessage());
        ...
        // Show the edit modal
        var editModal = new bootstrap.Modal(document.getElementById('editClostridiosisModal'));
        ...
        $('#saveEditClostridiosis').click(function() {
            var formData = {
                id: $('#edit_id').val(),
            ...
            Swal.fire({
                ...
                text: `¿Desea actualizar el registro de clostridiosis para el animal con Tag ID ${formData.tagid}?`,
                icon: 'question',
                showCancelButton: true,
                confirmButtonColor: '#28a745',
                cancelButtonColor: '#dc3545',
    ...
    $('.delete-clostridiosis').click(function() {
        var id = $(this).data('id');
        var tagid = $(this).data('tagid');
    ...
    $(document).on('click', '.register-new-clostridiosis-btn', function() { 
...
<!-- Clostridiosis Line Chart Section -->
    ...
    <div class="card shadow-lg border-0">
        ...
        <div class="card-body p-4">
            ...
            <div class="chart-container">
                <canvas id="clostridiosisChart"></canvas>
            </div>
        </div>
    </div>
</div>

<!-- Professional CSS Styling for Clostridiosis Charts -->
<style>
...
<!-- Chart.js Script for Clostridiosis Line Chart -->
<script>
$(document).ready(function() {
    let allClostridiosisData = [];
    let clostridiosisChart = null;
    
    // Add loading state to chart container
    $('.chart-container').addClass('loading');
    
    // Fetch clostridiosis data and create the chart
    $.ajax({
        url: 'get_clostridiosis_data.php',
        type: 'GET',
        dataType: 'json',
        beforeSend: function() {
            $('.chart-container').addClass('loading');
        ...
        success: function(data) {
            ...
            allClostridiosisData = data;
            populateAnimalFilter(data);
            createClostridiosisChart(data);
        ...
        error: function(xhr, status, error) {
            $('.chart-container').removeClass('loading');
            console.error('Error fetching clostridiosis data:', error);
        }
    });
    ...
    function updateChart() {
        ...
        let filteredData = [...allClostridiosisData];
    ...
    function updateChartData(data) {
        if (clostridiosisChart) {
            clostridiosisChart.destroy();
        }
        createClostridiosisChart(data);
    }
    
    function createClostridiosisChart(data) {
        var ctx = document.getElementById('clostridiosisChart').getContext('2d');
        ...
        clostridiosisChart = new Chart(ctx, {
            type: 'line',
            data: {
                labels: labels,
                datasets: [{
...
<div class="container mt-5 mb-5">
    <h4 class="text-center text-black mb-4 fw-bold">
        ...
        Estado de Vacunación Clostridiosis por Grupos
    </h4>
...
<!-- Chart.js Script for Clostridiosis Status Pie Charts -->
<script>
$(document).ready(function() {
    const clostridiosisStatusUrl = 'get_clostridiosis_status_by_grupo.php';
                ...
                <div class="card shadow-lg border-0 pie-chart-card">
                    ...
                    <div class="card-body p-4">
                        <div class="chart-container" style="position: relative; height:40vh; width:100%">
                            <canvas id="clostridiosisPie${safeGrupo}"></canvas>
                            <div id="clostridiosisPie${safeGrupo}Message" class="text-center small text-muted mt-2"></div>
                        </div>
    ...
    $.ajax({
        url: clostridiosisStatusUrl,
        type: 'GET',
        dataType: 'json',
        success: function(data) {
            if (data.error) {
                console.error('Error fetching clostridiosis status data:', data.error);
            ...
            grupos.forEach((grupo, index) => {
                ...
                const canvasId = `clostridiosisPie${safeGrupo}`;
                const messageId = `clostridiosisPie${safeGrupo}Message`;
        ...
        error: function(xhr, status, error) {
            console.error('AJAX error fetching clostridiosis status data:', error);
