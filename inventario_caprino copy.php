...
require_once './pdo_conexion.php';  // Go up one directory since inventario_caprino.php is in the caprino folder
...
$sql = "SELECT id, tagid, nombre, genero, raza, etapa, grupo, estatus, fecha_nacimiento, image, image2, image3, video 
        FROM caprino" . $where_clause . " ORDER BY tagid ASC";
...
// Sexo Counts
$sexoCountsQuery = "SELECT genero, COUNT(*) as count FROM caprino" . $where_clause . " GROUP BY genero";
...
// Raza Counts
$razaCountsQuery = "SELECT raza, COUNT(*) as count FROM caprino" . $where_clause . " GROUP BY raza";
...
// Etapa Counts
$etapaCountsQuery = "SELECT etapa, COUNT(*) as count FROM caprino" . $where_clause . " GROUP BY etapa";
...
// Grupos Counts
$grupoCountsQuery = "SELECT grupo, COUNT(*) as count FROM caprino" . $where_clause . " GROUP BY grupo";
...
// Estatus Counts
$estatusCountsQuery = "SELECT estatus, COUNT(*) as count FROM caprino" . $where_clause . " GROUP BY estatus";
...
$tagidsQuery = "SELECT tagid FROM caprino" . $where_clause;
...
$avgPesoQuery = "
    SELECT 
        DATE_FORMAT(ch_peso_fecha, '%Y-%m') AS peso_month, 
        AVG(ch_peso_animal) AS average_weight 
    FROM 
        ch_peso 
    WHERE 
        ch_peso_tagid IN (" . implode(',', $tagids) . ") 
    GROUP BY 
        peso_month 
    ORDER BY 
        peso_month ASC
...
$avgLecheQuery = "
    SELECT 
        DATE_FORMAT(ch_leche_fecha_inicio, '%Y-%m') AS leche_month, 
        AVG(ch_leche_peso) AS average_leche 
    FROM 
        ch_leche
    WHERE 
        ch_leche_tagid IN (" . implode(',', $tagids) . ") 
    GROUP BY 
        leche_month 
    ORDER BY 
        leche_month ASC
...
// Fetch average monthly ch_concentrado_racion * ch_concentrado_costo from ch_concentrado table
$avgRacionQuery = "
    SELECT 
        DATE_FORMAT(ch_concentrado_fecha_inicio, '%Y-%m') AS racion_month,
        AVG(ch_concentrado_racion * ch_concentrado_costo) AS average_racion_cost
    FROM 
        ch_concentrado
    GROUP BY 
        racion_month
...
// Fetch unique tagids from cc__historicos_tareas_partos
$tagidQuery = "SELECT DISTINCT ch_parto_tagid FROM ch_parto";
$tagidResult = $conn->query($tagidQuery);

$tagids = [];
if ($tagidResult && $tagidResult->num_rows > 0) {
    while ($row = $tagidResult->fetch_assoc()) {
        $tagids[] = $row['ch_parto_tagid'];
    }
}
...
$partoQuery = "
     SELECT 
         DATE_FORMAT(ch_parto_fecha, '%Y-%m') AS yearmonth,
         ch_parto_tagid,
         SUM(ch_parto_numero) AS total_parto
     FROM 
         ch_parto
     GROUP BY 
         DATE_FORMAT(ch_parto_fecha, '%Y-%m'), ch_parto_tagid
     ORDER BY 
         DATE_FORMAT(ch_parto_fecha, '%Y-%m') ASC
";
...
<head>
    ...
    <title>Inventario Caprino</title>
    <!-- Link to the Favicon -->
...
<style>
    ...
    /* Make carousel control backgrounds transparent */
    .cards-container .carousel-control-prev,
    .cards-container .carousel-control-next {
        background: transparent !important;
        border: none !important;
    }
    
    .cards-container .carousel-control-prev:hover,
    .cards-container .carousel-control-next:hover {
        background: transparent !important;
    }
    
    .cards-container .carousel-control-prev:focus,
    .cards-container .carousel-control-next:focus {
        background: transparent !important;
    }
    
    /* Hide carousel indicators (pagination dots) */
    .cards-container .carousel-indicators {
...
<script>
// Image preview function for both new and update modals
function previewImage(event, previewId) {
    ...
    reader.onload = function() {
        ...
        if (previewId.includes('update')) {
            // For update modal
            const carouselItems = document.querySelectorAll('#updateImagePreviewCarousel .carousel-item');
            carouselItems.forEach((item, index) => {
                if (item.querySelector('img') && item.querySelector('img').id === previewId) {
                    const carousel = bootstrap.Carousel.getInstance(document.getElementById('updateImagePreviewCarousel'));
                    carousel.to(index);
                }
            });
        } else {
...
function previewVideo(event, previewId) {
    ...
    reader.onload = function() {
        ...
        // Update source
        output.querySelector('source').src = reader.result;
        output.load(); // Important to reload the video element
        
        // Show the correct carousel item (last one for video)
        if (previewId.includes('update')) {
            const carousel = bootstrap.Carousel.getInstance(document.getElementById('updateImagePreviewCarousel'));
            const carouselItems = document.querySelectorAll('#updateImagePreviewCarousel .carousel-item');
...
function openFullscreenVideo(videoElement) {
    ...
    fullscreenVideo.src = videoElement.querySelector('source').src;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Modal event listener will be added after DOM is loaded
});
</script>
    ...
    <div id="floatingPromptMenu" class="floating-prompt-menu hidden">
        ...
        <div class="floating-menu-content">
            <div class="prompt-buttons">
                ...
                <button class="prompt-btn" onclick="selectPromptFromMenu('Produccion lechera')">
                    <i class="fas fa-tint"></i> Produccion lechera
                </button>
...
<script>
    ...
    function registrar(tagid) {
        if (confirm('¿Desea generar un informe completo para este animal?')) {
            ...
            window.location.href = 'caprino_report.php?tagid=' + encodeURIComponent(tagid);
        }
    }
</script>
<script>
    ...
    function openUpdateModal(tagid) {
        ...
        if (typeof $ === 'undefined') {
            ...
            alert('Error: jQuery library is not loaded');
            return;
        }
        ...
        console.log('Starting AJAX request to caprino_get_details.php');
        
        $.ajax({
            url: 'caprino_get_details.php',
            type: 'GET',
            data: { tagid: tagid },
            dataType: 'json',
            ...
            success: function(response) {
                ...
                if (!response.success) {
                    ...
                    alert('Error al cargar los datos del animal: ' + response.message);
                    return;
                }
                ...
                if (!data) {
                    ...
                    alert('Error: No se recibieron datos del animal');
                    return;
                }
                ...
                if (updateImagePreview) {
                    if (data.image && data.image.trim() !== '') {
                        // Remove 'uploads/' prefix if it exists, then add './uploads/'
                        const imagePath = data.image.replace(/^uploads\//, '');
                        updateImagePreview.src = './uploads/' + imagePath;
                    } else {
                ...
                if (updateImage2Preview) {
                    if (data.image2 && data.image2.trim() !== '') {
                        // Remove 'uploads/' prefix if it exists, then add './uploads/'
                        const image2Path = data.image2.replace(/^uploads\//, '');
                        updateImage2Preview.src = './uploads/' + image2Path;
                    } else {
                ...
                if (updateImage3Preview) {
                    if (data.image3 && data.image3.trim() !== '') {
                        // Remove 'uploads/' prefix if it exists, then add './uploads/'
                        const image3Path = data.image3.replace(/^uploads\//, '');
                        updateImage3Preview.src = './uploads/' + image3Path;
                    } else {
                ...
                if (updateVideoPreview) {
                    if (data.video && data.video.trim() !== '') {
                        ...
                        if (videoSource) {
                            // Remove 'uploads/' prefix if it exists, then add './uploads/'
                            const videoPath = data.video.replace(/^uploads\//, '');
                            videoSource.src = './uploads/' + videoPath;
                ...
                console.log('Setting animal ID to:', data.id);
                ...
                setFieldValue('updatePesoCompra', data.peso_compra);
                setFieldValue('updatePrecioCompra', data.precio_compra);

                // Show the modal
                modal.show();
                
                // Add modal event listener for carousel initialization
            ...
            error: function(xhr, status, error) {
                ...
                // Use SweetAlert2 for better error display
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-8">
                            ...
                            <div class="row g-3">
                                ...
                                <div class="col-md-6">
                                    <div class="form-floating">
                                        <select class="form-select" name="raza" id="updateRaza" required>
                                            ...
                                            $sql_razas = "SELECT DISTINCT cc_razas_nombre FROM cc_razas ORDER BY cc_razas_nombre";
                                            $result_razas = $conn_razas->query($sql_razas);
                                            while ($row_razas = $result_razas->fetch_assoc()) {
                                                echo '<option value="' . htmlspecialchars($row_razas['cc_razas_nombre']) . '">' 
                                                    . htmlspecialchars($row_razas['cc_razas_nombre']) . '</option>';
                                            }
                                            $conn_razas->close();
                                            ?>
                                        </select>
                                ...
                                <div class="col-md-6">
                                    <div class="form-floating">
                                        <select class="form-select" name="etapa" id="updateEtapa" required>
                                            ...
                                            $sql_etapa = "SELECT DISTINCT cc_etapas_nombre FROM cc_etapas ORDER BY cc_etapas_nombre";
                                            $result_etapa = $conn_etapa->query($sql_etapa);
                                            while ($row_etapa = $result_etapa->fetch_assoc()) {
                                                echo '<option value="' . htmlspecialchars($row_etapa['cc_etapas_nombre']) . '">' 
                                                    . htmlspecialchars($row_etapa['cc_etapas_nombre']) . '</option>';
                                            }
                                            $conn_etapa->close();
                                            ?>
                                        </select>
                                ...
                                <div class="col-md-6">
                                    <div class="form-floating">
                                        <select class="form-select" name="grupo" id="updateGrupo" required>
                                            ...
                                            $sql_grupos = "SELECT DISTINCT cc_grupos_nombre FROM cc_grupos ORDER BY cc_grupos_nombre";
                                            $result_grupos = $conn_grupos->query($sql_grupos);
                                            while ($row_grupos = $result_grupos->fetch_assoc()) {
                                                echo '<option value="' . htmlspecialchars($row_grupos['cc_grupos_nombre']) . '">' 
                                                    . htmlspecialchars($row_grupos['cc_grupos_nombre']) . '</option>';
                                            }
                                            $conn_grupos->close();
                                            ?>
                                        </select>
                                ...
                                <div class="col-md-6">
                                    <div class="form-floating">
                                        <select class="form-select" name="estatus" id="updateEstatus" required>
                                            ...
                                            $sql_estatus = "SELECT DISTINCT cc_estatus_nombre FROM cc_estatus ORDER BY cc_estatus_nombre";
                                            $result_estatus = $conn_estatus->query($sql_estatus);
                                            while ($row_estatus = $result_estatus->fetch_assoc()) {
                                                echo '<option value="' . htmlspecialchars($row_estatus['cc_estatus_nombre']) . '">' 
                                                    . htmlspecialchars($row_estatus['cc_estatus_nombre']) . '</option>';
                                            }
                                            $conn_estatus->close();
                                            ?>
                                        </select>
                                ...
                                <div class="col-md-6">
                                    <div class="form-floating">
                                        <input type="number" step="0.01" class="form-control" name="peso_compra" id="updatePesoCompra">
                                        <label for="updatePesoCompra">Peso de Compra (kg)</label>
                                    </div>
                                </div>
                                ...
                                <div class="col-md-6">
                                    <div class="form-floating">
                                        <input type="number" step="0.01" class="form-control" name="precio_compra" id="updatePrecioCompra">
                                        <label for="updatePrecioCompra">Precio de Compra ($)</label>
                                    </div>
                                </div>
                            </div>
                        </div>
...
function saveUpdates() {
    ...
    // Debug: Log the ID value being sent
    const animalId = document.getElementById('updateAnimalId');
    if (animalId) {
        console.log('Animal ID being sent:', animalId.value);
        if (!animalId.value) {
            console.error('Animal ID is empty!');
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: 'ID del animal no encontrado. Por favor, cierre el modal y vuelva a intentar.'
            });
            return;
        }
    } else {
        console.error('updateAnimalId element not found!');
        Swal.fire({
            icon: 'error',
            title: 'Error',
            text: 'Elemento ID del animal no encontrado en el formulario.'
        });
        return;
    }
...
async function searchAnimalAndGeneratePDF(query) {
    try {
        ...
        if (!contentType || !contentType.includes('application/json')) {
            ...
            console.error('Non-JSON response from search_animal.php:', text);
            throw new Error('Error del servidor al buscar animal. Por favor intenta nuevamente.');
        }
...
async function generateAnimalPDF(tagid) {
    try {
        ...
        const response = await fetch(`caprino_report.php?tagid=${encodeURIComponent(tagid)}`, {
            headers: {
        ...
        if (!contentType || !contentType.includes('application/json')) {
            const text = await response.text();
            console.error('Non-JSON response from caprino_report.php:', text);
            throw new Error('Error del servidor al generar PDF. Por favor intenta nuevamente.');
        }
        ...
        const menuHTML = `
            ...
            <div class="prompt-menu">
                ...
                <p style="margin-top: 15px; font-size: 0.9em; color: #666;">
                    O escribe tu propia consulta: Ejemplo. Muestra todos los registros de pesos, leche, concentrado, melaza, sal, vacunas, preñez, dias abiertos, descartes, sin parir, consumo de melaza, consumo de sal
                </p>
            </div>
        `;
...
function selectPromptFromMenu(promptText) {
    ...
    if (promptText === 'Razas' || promptText === 'Grupos' || promptText === 'Produccion Carnica' || promptText === 'Produccion lechera' || promptText === 'Consumo Concentrado' || promptText === 'Conversion' || promptText === 'Vacunas' || promptText === 'Preñez' || promptText === 'Dias Abiertos' || promptText === 'Descartes' || promptText === 'Sin parir' || promptText === 'sin parir' || promptText === 'Consumo Melaza' || promptText === 'Consumo Sal') {
        inputField.value = promptText;
...
async function sendChatMessage(mensaje) {
    try {
        ...
        } else if (mensaje === 'Produccion lechera') {
            promptPrefix = `Como veterinario especializado en ganado bovino, muestra todos los registros de la tabla Produccion Lechera y analiza la tendencia de la produccion de toda la finca y responde sobre las estadísticas de producción de leche. IMPORTANTE: No menciones números de tag ID en tu respuesta, ya que es una estadística general de toda la finca, no de un animal específico. Responde: `;
...
<script>
async function generateAndUploadPDF(tagid) {
    // Open the chat and automatically start the process for this animal
    chatBubble.classList.add('expanded');
    ...
    setTimeout(async () => {
        ...
        // Wait for chat to open, then automatically process this animal
        setTimeout(async () => {
            // Set the animal tagid in the input field and trigger search
