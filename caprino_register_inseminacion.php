...
$chartData = [];
try {
    $chartQuery = "SELECT DATE_FORMAT(ch_inseminacion_fecha, '%Y-%m') as month_year, COUNT(*) as count 
                     FROM ch_inseminacion 
                     GROUP BY month_year 
...
<title>Caprino Register Inseminacion</title>
<!-- Link to the Favicon -->
              ...
              <tbody>
                  ...
                  try {
                      // Query to get all Female animals and their LATEST insemination record (if any)
                        $inseminacionQuery = "
                            SELECT
                                ...
                                ins.ch_inseminacion_fecha,
                                ins.ch_inseminacion_tagid, -- Will be NULL if no insemination
                                ins.ch_inseminacion_numero,
                                ins.ch_inseminacion_costo
                            FROM
                                caprino v
                            LEFT JOIN ch_inseminacion ins
                              ON v.tagid = ins.ch_inseminacion_tagid
                            LEFT JOIN ch_inseminacion ins_check
                              ON ins.ch_inseminacion_tagid = ins_check.ch_inseminacion_tagid
                             AND ins.ch_inseminacion_fecha < ins_check.ch_inseminacion_fecha -- Check for a newer record
                            ...
                            ORDER BY
                                ...
                                ins.ch_inseminacion_fecha DESC, -- Then by latest insemination date
                      ...
                      if (empty($inseminacionData)) {
                          ...
                          try {
                              $configQuery = "SELECT e_vencimiento_inseminacion FROM e_vencimiento LIMIT 1";
                              $configStmt = $conn->prepare($configQuery);
                              $configStmt->execute();
                              
                              // Explicitly use PDO fetch method
                              $row = $configStmt->fetch(PDO::FETCH_ASSOC);
                              if ($row && isset($row['e_vencimiento_inseminacion'])) {
                                  $vigencia = intval($row['e_vencimiento_inseminacion']);
                              }
                          } catch (PDOException $e) {
                          ...
                          foreach ($inseminacionData as $row) {
                              ...
                              $inseminacionFecha = $row['ch_inseminacion_fecha'] ?? null;
                              ...
                              echo '<td class="text-center">
                                  <div class="btn-group" role="group">
                                      <button class="btn btn-success btn-sm" 
                                              ...
                                              data-tagid-prefill="'.htmlspecialchars($row['caprino_tagid'] ?? '').'"  -- Pass tagid to modal
                                      ...
                                      <button class="btn btn-warning btn-sm edit-inseminacion" 
                                          ...
                                          data-numero="'.htmlspecialchars($row['ch_inseminacion_numero'] ?? '').'" 
                                          data-costo="'.htmlspecialchars($row['ch_inseminacion_costo'] ?? '').'" 
                              ...
                              echo "<td class='text-center'>" . htmlspecialchars($row['caprino_tagid'] ?? 'N/A') . "</td>"; // Now using caprino_tagid
                              echo "<td class='text-center'>" . ($hasInseminacion ? htmlspecialchars($row['ch_inseminacion_numero'] ?? '') : 'N/A') . "</td>";
                              echo "<td class='text-center'>" . ($hasInseminacion ? htmlspecialchars($row['ch_inseminacion_costo'] ?? '') : 'N/A') . "</td>";
