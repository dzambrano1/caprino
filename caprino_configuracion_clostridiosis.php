...
<title>Caprino Configuracion Clostridiosis</title>
<!-- Link to the Favicon -->
...
<div class="container text-center">
  ...
  CONFIGURACION VACUNAS CLOSTRIDIOSIS
  </h3>
</div> 
<!-- New Entry Modal Configuracion Clostridiosis -->

<!-- Add New Vacuna Clostridiosis Button -->
<div class="container my-3 text-center">
  <button type="button" class="btn btn-success text-center" data-bs-toggle="modal" data-bs-target="#newEntryModal">
    <i class="fas fa-plus-circle me-2"></i>Nueva Vacuna Clostridiosis
  </button>
</div>
      ...
      <div class="modal-content">
          <div class="modal-header">
              <h5 class="modal-title" id="newEntryModalLabel">
                  <i class="fas fa-plus-circle me-2"></i>Configurar Nueva Vacuna Clostridiosis
              </h5>
...
<div class="container table-section" style="display: block;">
      <div class="table-responsive">
          <table id="clostridiosisTable" class="table table-striped table-bordered">
              ...
              <tbody>
                  <?php
                      $clostridiosisQuery = "SELECT * FROM cc_clostridiosis";
                      ...
                      if (empty($clostridiosissData)) {
                          ...
                          foreach ($clostridiosissData as $row) {
                              ...
                              echo '        <button class="btn btn-warning btn-sm edit-clostridiosis" 
                                              ...
                                              data-vacuna="' . htmlspecialchars($row['cc_clostridiosis_vacuna'] ?? '') . '" 
                                              data-dosis="' . htmlspecialchars($row['cc_clostridiosis_dosis'] ?? '') . '" 
                                              data-costo="' . htmlspecialchars($row['cc_clostridiosis_costo'] ?? '') . '" 
                                              data-vigencia="' . htmlspecialchars($row['cc_clostridiosis_vigencia'] ?? '') . '"
                                              title="Editar Configuracion Vacuna Clostridiosis">
                                              <i class="fas fa-edit"></i>
                                          </button>';
                              echo '        <button class="btn btn-danger btn-sm delete-clostridiosis" 
                                              data-id="' . htmlspecialchars($row['id'] ?? '') . '"
                                              title="Eliminar Configuracion Vacuna Clostridiosis">
                              ...
                              // Column 1: Vacuna
                              echo "<td>" . htmlspecialchars($row['cc_clostridiosis_vacuna'] ?? '') . "</td>";
                              
                              // Column 2: Dosis
                              echo "<td>" . htmlspecialchars($row['cc_clostridiosis_dosis'] ?? 'N/A') . "</td>";
                              
                              // Column 3: Costo
                              echo "<td>" . htmlspecialchars($row['cc_clostridiosis_costo'] ?? 'N/A') . "</td>";
                              
                              // Column 4: Vigencia
                              echo "<td>" . htmlspecialchars($row['cc_clostridiosis_vigencia'] ?? 'N/A') . "</td>";
...
$(document).ready(function() {
    ...
    $('#saveNewClostridiosis').click(function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: 'process_configuracion_clostridiosis.php',
                    type: 'POST',
                    data: {
                        action: 'insert',
    ...
    $('.edit-clostridiosis').click(function() {
        ...
        // Edit Configuracion Clostridiosis Modal dialog for editing
        var modalHtml = `
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editClostridiosisModalLabel">
                            <i class="fas fa-weight me-2"></i>Editar Vacuna Clostridiosis
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
        ...
        $('#saveEditClostridiosis').click(function() {
            ...
            var form = document.getElementById('editClostridiosisForm');
            if (!form.checkValidity()) {
                form.reportValidity();
                return;
            }
            ...
            Swal.fire({
                ...
                text: `¿Desea actualizar la configuracion de clostridiosis?`,
                icon: 'question',
                showCancelButton: true,
                confirmButtonColor: '#28a745',
                cancelButtonColor: '#dc3545',
    ...
    $('.delete-clostridiosis').click(function() {
        ...
        Swal.fire({
            ...
            text: `¿Está seguro de que desea eliminar la configuracion de clostridiosis? Esta acción no se puede deshacer.`,
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#dc3545',
