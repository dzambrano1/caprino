...
$avgPesoQuery = "
    ...
    FROM 
        ch_peso 
    WHERE 
        ch_peso_tagid IN (" . str_repeat('?,', count($tagids) - 1) . "?)
    GROUP BY 
        peso_month 
    ORDER BY 
        peso_month ASC
...
// Fetch unique tagids from cc_historicos_tareas_partos
...
window.generateAndUploadPDF = async function(tagid) {
    ...
    // Make AJAX request to generate and upload PDF using caprino_report.php
    $.ajax({
        url: 'caprino_report.php',
        type: 'GET',
        data: {
            tagid: tagid,
            upload_to_chatpdf: '1'
        ...
        success: function(response) {
            ...
            if (response.success) {
                console.log('PDF generation successful, checking upload result...', response);
                
                // Check if ChatPDF upload was successful
                if (response.upload_result && response.upload_result.success && response.upload_result.sourceId) {
                    console.log('ChatPDF upload successful, opening chat window...', response.upload_result.sourceId);
                    
                    // Show brief success message and automatically open chat
                    Swal.fire({
                        icon: 'success',
                        title: '¡PDF Generado y Subido a IA!',
                        text: 'Abriendo chat veterinario...',
                        timer: 1500,
                        showConfirmButton: false
                    });
                    ...
                    setTimeout(() => {
                        ...
                        console.log('Opening ChatPDF window with sourceId:', sourceId);
                        openChatPDFWindow(sourceId, response.filename);
                    }, 1000);
                    
                } else {
                    // Upload failed or no sourceId, show options dialog
                    console.log('ChatPDF upload failed or no sourceId, showing options...', response.upload_result);
                    ...
                    Swal.fire({
                        ...
                        html: `
                            ...
                            <p><strong>Estado de subida a IA:</strong> ${response.upload_result && response.upload_result.success ? 'Exitoso' : 'Fallido'}</p>
                            ${response.upload_result && response.upload_result.error ? `<p><strong>Error:</strong> ${response.upload_result.error}</p>` : ''}
                            <p>El PDF se generó correctamente, pero no se pudo subir a la IA.</p>
                        `,
                        ...
                        const directUrl = window.location.origin + window.location.pathname.replace('inventario_caprino.php', '') + 'reports/' + response.filename;
                        const viewerUrl = window.location.origin + window.location.pathname.replace('inventario_caprino.php', '') + 'view_pdf.php?file=' + response.filename;
...
<script>
    ...
    function openUpdateModal(tagid) {
        ...
        console.log('Starting AJAX request to fetch_caprino_data.php');
        
        $.ajax({
            url: 'fetch_caprino_data.php',
            type: 'GET',
            data: { tagid: tagid },
                                ...
                                <div class="col-md-6">
                                    <div class="form-floating">
                                        <select class="form-select" name="etapa" id="updateEtapa" required>
                                            <option value="">Seleccionar</option>
                                            <?php
                                            $sql_etapa = "SELECT DISTINCT etapa FROM caprino ORDER BY etapa";
