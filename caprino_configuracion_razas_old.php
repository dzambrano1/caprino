...
<title>Caprino Configuracion Razas</title>
<!-- Link to the Favicon -->
...
<div class="modal fade" id="newEntryModal" tabindex="-1" aria-labelledby="newEntryModalLabel">
  ...
  <!-- DataTable for cc_razas records -->
...
<div class="container table-section" style="display: block;">
      <div class="table-responsive">
          <table id="razasTable" class="table table-striped table-bordered">
              ...
              <tbody>
                  <?php
                      $razasQuery = "SELECT * FROM cc_razas";
                      ...
                      if (empty($razassData)) {
                          ...
                          foreach ($razassData as $row) {
                              ...
                              echo '        <button class="btn btn-warning btn-sm edit-razas me-1" 
                                                  data-id="' . htmlspecialchars($row['id'] ?? '') . '"
                                                  data-razas="' . htmlspecialchars($row['cc_razas_nombre'] ?? '') . '"
                                                  title="Editar Configuracion Raza">
                                                  <i class="fas fa-edit"></i>
                                              </button>';
                              echo '        <button class="btn btn-danger btn-sm delete-razas" 
                                                  data-id="' . htmlspecialchars($row['id'] ?? '') . '"
                                                  data-razas="' . htmlspecialchars($row['cc_razas_nombre'] ?? '') . '"
                                                  title="Eliminar Configuracion Raza">
                              ...
                              // Column 1: Vacuna
                              echo "<td>" . htmlspecialchars($row['cc_razas_nombre'] ?? '') . "</td>";
                                ...
                                <div class="input-group">
                                    <span class="input-group-text">
                                        ...
                                        <select class="form-select" id="edit_razas" name="razas" required>
                                            ...
                                            $sql_names = "SELECT DISTINCT cc_razas_nombre FROM cc_razas ORDER BY cc_razas_nombre ASC";
                                            $stmt_names = $conn->prepare($sql_names);
                                            $stmt_names->execute();
                                            $names = $stmt_names->fetchAll(PDO::FETCH_ASSOC);
                                            foreach ($names as $name_row) {
                                                echo '<option value="' . htmlspecialchars($name_row['cc_razas_nombre']) . '">' . htmlspecialchars($name_row['cc_razas_nombre']) . '</option>';
                                            }
                                            ?>
                                        </select>
                                    </span>                                    
                    ...
                    error: function(xhr, status, error) {
                        ...
                        try {
                            ...
                            if (response.message) {
                                ...
                                if (response.message.includes('No se puede eliminar esta raza porque está siendo utilizada')) {
                                    errorTitle = 'No se puede eliminar';
                                    errorMsg = response.message + '\n\nPara eliminar esta raza, primero debe:';
                                    errorMsg += '\n1. Cambiar la raza de todos los animales que la usan';
                                    errorMsg += '\n2. O eliminar los registros que la referencian';
                                }
                            }
                        } catch (e) {
                            // If it's not valid JSON, it might be an HTML error page
