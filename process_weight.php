...
if (isset($_POST['action'])) {
    ...
    try {
        ...
        if ($action === 'insert') {
            ...
            // Execute query
            $stmt = $conn->prepare("INSERT INTO ch_peso (ch_peso_tagid, ch_peso_animal, ch_peso_precio, ch_peso_fecha) VALUES (?, ?, ?, ?)");
        ...
        // UPDATE operation
            ...
            // Execute query
            $stmt = $conn->prepare("UPDATE ch_peso SET ch_peso_tagid = ?, ch_peso_animal = ?, ch_peso_precio = ?, ch_peso_fecha = ? WHERE id = ?");
        ...
        // DELETE operation
            ...
            // Execute query
            $stmt = $conn->prepare("DELETE FROM ch_peso WHERE id = ?");
            $stmt->execute([$id]);
