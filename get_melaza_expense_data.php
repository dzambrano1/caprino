...
try {
    ...
    // SQL query to get individual melaza records with actual start and end dates for proper cross-month distribution
    $sql = "SELECT 
                ch_melaza_racion,
                ch_melaza_costo,
    ...
    if (mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            $totalExpense = (float)$row['ch_melaza_racion'] * (float)$row['ch_melaza_costo'] * $totalDays;
            ...
            while ($currentDate <= $endDate) {
                ...
                $monthlyData[$monthKey]['total_quantity'] += (float)$row['ch_melaza_racion'];
                $monthlyData[$monthKey]['total_cost_sum'] += (float)$row['ch_melaza_costo'];
