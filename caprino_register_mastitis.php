...
<title>Caprino - Registro de Vacunación Mastitis</title>
<!-- Link to the Favicon -->
...
<div class="container text-center">
                        ...
                        <div class="input-group">
                            <span class="input-group-text">
                                ...
                                <select class="form-select" id="new_vacuna" name="vacuna" required>
                                    ...
                                    <?php
                                    try {
                                        $sql_vacunas = "SELECT DISTINCT cc_mastitis_vacuna FROM cc_mastitis ORDER BY cc_mastitis_vacuna ASC";
                                        ...
                                        foreach ($vacunas as $vacuna_row) {
                                            echo '<option value="' . htmlspecialchars($vacuna_row['cc_mastitis_vacuna']) . '">' . htmlspecialchars($vacuna_row['cc_mastitis_vacuna']) . '</option>';
                                        }
  ...
  <!-- DataTable for ch_mastitis records -->
              ...
              <tbody>
                  ...
                  try {
                      $mastitisQuery = "SELECT 
                                          ...
                                          v.tagid,
                                          v.nombre,
                                          va.ch_mastitis_fecha,
                                          va.id AS mastitis_record_id, -- Alias mastitis record ID
                                          va.ch_mastitis_producto,
                                          va.ch_mastitis_dosis,
                                          va.ch_mastitis_costo,
                                          -- Flag to easily check if there's a history record
                                          CASE WHEN va.ch_mastitis_tagid IS NOT NULL THEN 1 ELSE 0 END AS in_mastitis_history
                                      FROM 
                                          caprino v
                                      LEFT JOIN 
                                          -- Join with all mastitis records per tagid
                                          ch_mastitis va ON v.tagid = va.ch_mastitis_tagid 
                                      WHERE 
                                          v.genero = 'Hembra'
                                          AND va.ch_mastitis_tagid IS NOT NULL
                                      ORDER BY 
                                          va.ch_mastitis_fecha DESC, v.tagid ASC;";
                      ...
                      if (empty($mastitissData)) {
                          ...
                          foreach ($mastitissData as $row) {
                              ...
                              echo '        <button class="btn btn-warning btn-sm edit-mastitis" 
                                                  ...
                                                  data-vacuna="' . htmlspecialchars($row['ch_mastitis_producto'] ?? '') . '" 
                                                  data-dosis="' . htmlspecialchars($row['ch_mastitis_dosis'] ?? '') . '" 
                                                  data-costo="' . htmlspecialchars($row['ch_mastitis_costo'] ?? '') . '" 
                                                  data-fecha="' . htmlspecialchars($row['ch_mastitis_fecha'] ?? '') . '"
                                                  title="Editar Vacuna">
                              ...
                              echo "<td>" . htmlspecialchars($row['ch_mastitis_fecha'] ?? '') . "</td>";
                              ...
                              echo "<td>" . htmlspecialchars($row['ch_mastitis_producto'] ?? 'N/A') . "</td>";
                              echo "<td>" . htmlspecialchars($row['ch_mastitis_dosis'] ?? 'N/A') . "</td>";
                              echo "<td>" . htmlspecialchars($row['ch_mastitis_costo'] ?? 'N/A') . "</td>";

                              // Calculate due date and determine status
                              try {
                                  if (!empty($row['ch_mastitis_fecha'])) {
                                      $mastitisDate = new DateTime($row['ch_mastitis_fecha']);
                                  ...
                                  error_log("Date error: " . $e->getMessage() . " for date: " . $row['ch_mastitis_fecha']);
                        ...
                        try {
                             ...
                             $noMastitisQuery = "SELECT 
                                                 ...
                                                 WHERE 
                                                     v.genero = 'Hembra'
                                                     AND v.tagid NOT IN (
                                                         SELECT DISTINCT ch_mastitis_tagid 
                                                         FROM ch_mastitis 
                                                         WHERE ch_mastitis_tagid IS NOT NULL
                                                     )
                                                 ORDER BY 
                        ...
                        <div class="mb-3">
                            ...
                            <div class="list-group" id="vacunaList" style="max-height: 300px; overflow-y: auto;">
                                ...
                                try {
                                    ...
                                    $vacunas = $stmt_vacunas->fetchAll(PDO::FETCH_ASSOC);
                                    foreach ($vacunas as $vacuna_row) {
                                        echo '<button type="button" class="list-group-item list-group-item-action vacuna-option" data-value="' . htmlspecialchars($vacuna_row['cc_mastitis_vacuna']) . '">' . htmlspecialchars($vacuna_row['cc_mastitis_vacuna']) . '</button>';
                                    }
                                } catch (PDOException $e) {
                                    error_log("Error fetching vacunas: " . $e->getMessage());
