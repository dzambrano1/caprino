              ...
              <tbody>
                  ...
                  try {
                      ...
                      if (empty($concentradoData)) {
                          ...
                          try {
                              $configQuery = "SELECT e_vencimiento_concentrado FROM e_vencimiento LIMIT 1";
                              $configStmt = $conn->prepare($configQuery);
                              $configStmt->execute();
                              
                              // Explicitly use PDO fetch method
                              $row = $configStmt->fetch(PDO::FETCH_ASSOC);
                              if ($row && isset($row['e_vencimiento_concentrado'])) {
                                  $vigencia = intval($row['e_vencimiento_concentrado']);
                              }
                          } catch (PDOException $e) {
...
<div class="container chart-container mb-4">
    <div class="card shadow-lg border-0">
        ...
        <div class="card-body p-4">
            ...
            // Immediate test - create a simple visible chart
            setTimeout(function() {
                ...
                if (canvas) {
                    ...
                    ctx.fillStyle = 'blue';
                    ctx.font = '16px Arial';
                    ctx.fillText('TEST CHART', 20, 40);
                    
                    console.log('Test chart drawn');
                } else {
