...
try {
    ...
    $salQuery = "SELECT 
                        ...
                        AND ch_sal_racion IS NOT NULL 
                        AND ch_sal_costo IS NOT NULL
                        ORDER BY ch_sal_fecha_inicio ASC";
    ...
    if (empty($salData)) {
        ...
        $salQuery = "SELECT 
                                ch_sal_fecha,
                                ch_sal_fecha as ch_sal_fecha_inicio,
                                ch_sal_fecha as ch_sal_fecha_fin,
                                ch_sal_racion,
                                ch_sal_costo,
                                ch_sal_tagid
                            FROM ch_sal 
                            WHERE ch_sal_fecha IS NOT NULL 
                            AND ch_sal_racion IS NOT NULL 
                            AND ch_sal_costo IS NOT NULL
                            ORDER BY ch_sal_fecha ASC";
    ...
    foreach ($salData as $sal) {
        $startDate = new DateTime($sal['ch_sal_fecha_inicio']);
        $endDate = new DateTime($sal['ch_sal_fecha_fin']);
        
        // Calculate total expense value
        $totalDays = $endDate->diff($startDate)->days + 1;
        $totalValue = $sal['ch_sal_racion'] * $sal['ch_sal_costo'] * $totalDays;
        $dailyValue = $totalValue / $totalDays;
        
        error_log("Processing sal: Start=" . $sal['ch_sal_fecha_inicio'] . 
                 ", End=" . $sal['ch_sal_fecha_fin'] . 
                 ", Days=" . $totalDays . 
...
<title>caprino - Registro de Sal</title>
<!-- Link to the Favicon -->
                        ...
                        <div class="input-group">
                            <span class="input-group-text">
                                ...
                                <select class="form-select" id="new_alimento" name="alimento" required>
                                    ...
                                    <?php
                                    try {
                                        $sql_alimentos = "SELECT DISTINCT cc_sal_nombre FROM cc_sal ORDER BY cc_sal_nombre ASC";
                                        ...
                                        foreach ($alimentos as $alimento_row) {
                                            echo '<option value="' . htmlspecialchars($alimento_row['cc_sal_nombre']) . '">' . htmlspecialchars($alimento_row['cc_sal_nombre']) . '</option>';
                                        }
                                    } catch (PDOException $e) {
        ...
        <div class="table-responsive">
            ...
            try {
                      // Query to get all Animals and ALL their Sal records (if any)
                        $salQuery = "
                            SELECT
                                ...
                                c.ch_sal_fecha_inicio,
                                c.ch_sal_fecha_fin,
                                c.ch_sal_tagid,         -- Matches caprino_tagid if sal exists
                                c.ch_sal_etapa,
                                c.ch_sal_producto,
                                c.ch_sal_racion,
                                c.ch_sal_costo,
                                -- Calculate total_value only if c.id is not null
                                CASE WHEN c.id IS NOT NULL THEN CAST((c.ch_sal_racion * c.ch_sal_costo) AS DECIMAL(10,2)) ELSE NULL END as total_value
                            FROM
                                caprino v
                            LEFT JOIN
                                ch_sal c ON v.tagid = c.ch_sal_tagid -- Join ALL matching sal records
                            ORDER BY
                      ...
                      if (empty($salData)) {
                          ...
                          try {
                              $configQuery = "SELECT c_vencimiento_sal FROM c_vencimiento LIMIT 1";
                              $configStmt = $conn->prepare($configQuery);
                              $configStmt->execute();
                              
                              // Explicitly use PDO fetch method
                              $row = $configStmt->fetch(PDO::FETCH_ASSOC);
                              if ($row && isset($row['c_vencimiento_sal'])) {
                                  $vigencia = intval($row['c_vencimiento_sal']);
                              }
                          } catch (PDOException $e) {
                          ...
                          foreach ($salData as $row) {
                              ...
                              $salFecha = $row['ch_sal_fecha_inicio'] ?? null;
                              $salFechaInicio = $row['ch_sal_fecha_inicio'] ?? null;
                              $salFechaFin = $row['ch_sal_fecha_fin'] ?? null;
                              ...
                              if ($hasSal) {
                                  ...
                                  echo '        <button class="btn btn-warning btn-sm edit-sal" 
                                                  ...
                                                  data-producto="'.htmlspecialchars($row['ch_sal_producto'] ?? '').'" 
                                                  data-racion="'.htmlspecialchars($row['ch_sal_racion'] ?? '').'" 
                                                  data-costo="'.htmlspecialchars($row['ch_sal_costo'] ?? '').'" 
                                                  data-fecha-inicio="'.htmlspecialchars($salFechaInicio ?? '').'"
                                                  data-fecha-fin="'.htmlspecialchars($salFechaFin ?? '').'"
                                                  title="Editar Registro"
                                                  data-debug="DB: '.htmlspecialchars($row['ch_sal_producto'] ?? 'NULL').'">
                              ...
                              echo "<td>" . ($hasSal ? htmlspecialchars($row['ch_sal_producto'] ?? '') : 'N/A') . "</td>";
                              // Column 7: Racion (or N/A)
                              echo "<td>" . ($hasSal ? htmlspecialchars($row['ch_sal_racion'] ?? '') : 'N/A') . "</td>";
                              // Column 8: Costo (or N/A)
                              echo "<td>" . ($hasSal ? htmlspecialchars($row['ch_sal_costo'] ?? '') : 'N/A') . "</td>";
                        ...
                        <div class="mb-3">
                            ...
                            <div class="list-group" id="productList" style="max-height: 300px; overflow-y: auto;">
                                ...
                                try {
                                    ...
                                    $alimentos = $stmt_alimentos->fetchAll(PDO::FETCH_ASSOC);
                                    foreach ($alimentos as $alimento_row) {
                                        echo '<button type="button" class="list-group-item list-group-item-action product-option" data-value="' . htmlspecialchars($alimento_row['cc_sal_nombre']) . '">' . htmlspecialchars($alimento_row['cc_sal_nombre']) . '</button>';
                                    }
                                } catch (PDOException $e) {
