...
<title>Equino - Registro de Vacunación Garrapatas</title>
<!-- Link to the Favicon -->
...
<div class="container text-center">
                        ...
                        <div class="input-group">
                            <span class="input-group-text">
                                ...
                                <select class="form-select" id="new_vacuna" name="vacuna" required>
                                    ...
                                    <?php
                                    try {
                                        $sql_vacunas = "SELECT DISTINCT cc_garrapatas_vacuna FROM cc_garrapatas ORDER BY cc_garrapatas_vacuna ASC";
                                        ...
                                        foreach ($vacunas as $vacuna_row) {
                                            echo '<option value="' . htmlspecialchars($vacuna_row['cc_garrapatas_vacuna']) . '">' . htmlspecialchars($vacuna_row['cc_garrapatas_vacuna']) . '</option>';
                                        }
  ...
  <!-- DataTable for ch_garrapatas records -->
              ...
              <tbody>
                  ...
                  try {
                      $garrapatasQuery = "SELECT 
                                          ...
                                          v.tagid,
                                          v.nombre,
                                          va.ch_garrapatas_fecha,
                                          va.id AS garrapatas_record_id, -- Alias garrapatas record ID
                                          va.ch_garrapatas_producto,
                                          va.ch_garrapatas_dosis,
                                          va.ch_garrapatas_costo,
                                          -- Flag to easily check if there's a history record
                                          CASE WHEN va.ch_garrapatas_tagid IS NOT NULL THEN 1 ELSE 0 END AS in_garrapatas_history
                                      FROM 
                                          caprino v
                                      LEFT JOIN 
                                          -- Join with all garrapatas records per tagid
                                          ch_garrapatas va ON v.tagid = va.ch_garrapatas_tagid 
                                      ORDER BY 
                                          va.ch_garrapatas_fecha DESC, v.tagid ASC;";
                      ...
                      if (empty($garrapatassData)) {
                          ...
                          try {
                              $configQuery = "SELECT e_vencimiento_garrapatas FROM e_vencimiento LIMIT 1";
                              $configStmt = $conn->prepare($configQuery);
                              $configStmt->execute();
                              $row_config = $configStmt->fetch(PDO::FETCH_ASSOC);
                              if ($row_config && isset($row_config['e_vencimiento_garrapatas'])) {
                                  $vigencia = intval($row_config['e_vencimiento_garrapatas']);
                              }
                          } catch (PDOException $e) {
                          ...
                          foreach ($garrapatassData as $row) {
                              ...
                              echo '        <button class="btn btn-success btn-sm register-new-garrapatas-btn" 
                              ...
                              if ($row['in_garrapatas_history'] == 1) {
                                  echo '        <button class="btn btn-warning btn-sm edit-garrapatas" 
                                                  ...
                                                  data-vacuna="' . htmlspecialchars($row['ch_garrapatas_producto'] ?? '') . '" 
                                                  data-dosis="' . htmlspecialchars($row['ch_garrapatas_dosis'] ?? '') . '" 
                                                  data-costo="' . htmlspecialchars($row['ch_garrapatas_costo'] ?? '') . '" 
                                                  data-fecha="' . htmlspecialchars($row['ch_garrapatas_fecha'] ?? '') . '"
                                                  title="Editar Vacuna">
                              ...
                              echo "<td>" . htmlspecialchars($row['ch_garrapatas_fecha'] ?? '') . "</td>";
                              echo "<td>" . htmlspecialchars($row['nombre'] ?? 'N/A') . "</td>";
                              echo "<td>" . htmlspecialchars($row['tagid'] ?? '') . "</td>";

                              if ($row['in_garrapatas_history'] == 1) {
                                  // Animal has vaccination history
                                  echo "<td>" . htmlspecialchars($row['ch_garrapatas_producto'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['ch_garrapatas_dosis'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['ch_garrapatas_costo'] ?? 'N/A') . "</td>";

                                  // Calculate due date and determine status
                                  try {
                                      if (!empty($row['ch_garrapatas_fecha'])) {
                                          $garrapatasDate = new DateTime($row['ch_garrapatas_fecha']);
                                      ...
                                      error_log("Date error: " . $e->getMessage() . " for date: " . $row['ch_garrapatas_fecha']);
                        ...
                        <div class="mb-3">
                            ...
                            <div class="list-group" id="vacunaList" style="max-height: 300px; overflow-y: auto;">
                                ...
                                try {
                                    ...
                                    $vacunas = $stmt_vacunas->fetchAll(PDO::FETCH_ASSOC);
                                    foreach ($vacunas as $vacuna_row) {
                                        echo '<button type="button" class="list-group-item list-group-item-action vacuna-option" data-value="' . htmlspecialchars($vacuna_row['cc_garrapatas_vacuna']) . '">' . htmlspecialchars($vacuna_row['cc_garrapatas_vacuna']) . '</button>';
                                    }
                                } catch (PDOException $e) {
                                    error_log("Error fetching vacunas: " . $e->getMessage());
