...
<title>caprino Configuracion Etapas</title>
<!-- Link to the Favicon -->
...
<div class="modal fade" id="newEntryModal" tabindex="-1" aria-labelledby="newEntryModalLabel">
  ...
  <!-- DataTable for cc_etapas records -->
...
<div class="container table-section" style="display: block;">
      <div class="table-responsive">
          <table id="etapasTable" class="table table-striped table-bordered">
              ...
              <tbody>
                  <?php
                      $etapasQuery = "SELECT * FROM cc_etapas";
                      ...
                      if (empty($etapassData)) {
                          ...
                          foreach ($etapassData as $row) {
                              ...
                              echo '        <button class="btn btn-danger btn-sm delete-etapas" 
                                              data-id="' . htmlspecialchars($row['id'] ?? '') . '"
                                              data-etapas="' . htmlspecialchars($row['cc_etapas_nombre'] ?? '') . '"
                              ...
                              // Column 1: Vacuna
                              echo "<td>" . htmlspecialchars($row['cc_etapas_nombre'] ?? '') . "</td>";
                                ...
                                <div class="input-group">
                                    <span class="input-group-text">
                                        ...
                                        <select class="form-select" id="edit_etapas" name="etapas" required>
                                            ...
                                            $sql_names = "SELECT DISTINCT cc_etapas_nombre FROM cc_etapas ORDER BY cc_etapas_nombre ASC";
                                            $stmt_names = $conn->prepare($sql_names);
                                            $stmt_names->execute();
                                            $names = $stmt_names->fetchAll(PDO::FETCH_ASSOC);
                                            foreach ($names as $name_row) {
                                                echo '<option value="' . htmlspecialchars($name_row['cc_etapas_nombre']) . '">' . htmlspecialchars($name_row['cc_etapas_nombre']) . '</option>';
                                            }
                                            ?>
                                        </select>
                                    </span>                                    
