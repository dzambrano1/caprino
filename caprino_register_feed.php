...
try {
    ...
    $concentradoQuery = "SELECT 
                            ch_concentrado_fecha_inicio,
                            ch_concentrado_fecha_fin,
                            ch_concentrado_racion,
                            ch_concentrado_costo,
                            ch_concentrado_tagid
                        FROM ch_concentrado 
                        WHERE ch_concentrado_fecha_inicio IS NOT NULL 
                        AND ch_concentrado_fecha_fin IS NOT NULL 
                        AND ch_concentrado_racion IS NOT NULL 
                        AND ch_concentrado_costo IS NOT NULL
    ...
    if (empty($concentradoData)) {
        ...
        $concentradoQuery = "SELECT 
                                ch_concentrado_fecha,
                                ch_concentrado_fecha as ch_concentrado_fecha_inicio,
                                ch_concentrado_fecha as ch_concentrado_fecha_fin,
                                ch_concentrado_racion,
                                ch_concentrado_costo,
                                ch_concentrado_tagid
                            FROM ch_concentrado 
                            WHERE ch_concentrado_fecha IS NOT NULL 
                            AND ch_concentrado_racion IS NOT NULL 
                            AND ch_concentrado_costo IS NOT NULL
                            ORDER BY ch_concentrado_fecha ASC";
    ...
    foreach ($concentradoData as $concentrado) {
        $startDate = new DateTime($concentrado['ch_concentrado_fecha_inicio']);
        $endDate = new DateTime($concentrado['ch_concentrado_fecha_fin']);
        
        // Calculate total expense value
        $totalDays = $endDate->diff($startDate)->days + 1;
        $totalValue = $concentrado['ch_concentrado_racion'] * $concentrado['ch_concentrado_costo'] * $totalDays;
        $dailyValue = $totalValue / $totalDays;
        
        error_log("Processing concentrado: Start=" . $concentrado['ch_concentrado_fecha_inicio'] . 
                 ", End=" . $concentrado['ch_concentrado_fecha_fin'] . 
                 ", Days=" . $totalDays . 
...
<title>caprino - Registro de Alimento Concentrado</title>
<!-- Link to the Favicon -->
...
<div class="container text-center">
                        ...
                        <div class="input-group">
                            <span class="input-group-text">
                                ...
                                <select class="form-select" id="new_alimento" name="alimento" required>
                                    ...
                                    <?php
                                    try {
                                        $sql_alimentos = "SELECT DISTINCT cc_concentrado_nombre FROM cc_concentrado ORDER BY cc_concentrado_nombre ASC";
                                        ...
                                        foreach ($alimentos as $alimento_row) {
                                            echo '<option value="' . htmlspecialchars($alimento_row['cc_concentrado_nombre']) . '">' . htmlspecialchars($alimento_row['cc_concentrado_nombre']) . '</option>';
                                        }
                                    } catch (PDOException $e) {
  ...
  <!-- DataTable for ch_concentrado records -->
              ...
              <tbody>
                  ...
                  try {
                      // Query to get all Animals and ALL their concentrado records (if any)
                        $concentradoQuery = "
                            SELECT
                                ...
                                v.etapa AS caprino_etapa,
                                c.id AS concentrado_id,         -- Will be NULL for animals with no concentrado records
                                c.ch_concentrado_fecha_inicio,
                                c.ch_concentrado_fecha_fin,
                                c.ch_concentrado_tagid,         -- Matches caprino_tagid if concentrado exists
                                c.ch_concentrado_etapa,
                                c.ch_concentrado_producto,
                                c.ch_concentrado_racion,
                                c.ch_concentrado_costo,
                                -- Calculate total_value only if c.id is not null
                                CASE WHEN c.id IS NOT NULL THEN CAST((c.ch_concentrado_racion * c.ch_concentrado_costo) AS DECIMAL(10,2)) ELSE NULL END as total_value
                            FROM
                                caprino v
                            LEFT JOIN
                                ch_concentrado c ON v.tagid = c.ch_concentrado_tagid -- Join ALL matching concentrado records
                            ORDER BY
                                ...
                                c.ch_concentrado_fecha_inicio DESC";
                      ...
                      if (empty($concentradoData)) {
                          ...
                          try {
                              $configQuery = "SELECT c_vencimiento_concentrado FROM c_vencimiento LIMIT 1";
                              $configStmt = $conn->prepare($configQuery);
                              $configStmt->execute();
                              
                              // Explicitly use PDO fetch method
                              $row = $configStmt->fetch(PDO::FETCH_ASSOC);
                              if ($row && isset($row['c_vencimiento_concentrado'])) {
                                  $vigencia = intval($row['c_vencimiento_concentrado']);
                              }
                          } catch (PDOException $e) {
                          ...
                          foreach ($concentradoData as $row) {
                              ...
                              $concentradoFecha = $row['ch_concentrado_fecha_inicio'] ?? null;
                              $concentradoFechaInicio = $row['ch_concentrado_fecha_inicio'] ?? null;
                              $concentradoFechaFin = $row['ch_concentrado_fecha_fin'] ?? null;
                              ...
                              if ($hasConcentrado) {
                                  ...
                                  echo '        <button class="btn btn-warning btn-sm edit-concentrado" 
                                                  ...
                                                  data-etapa="'.htmlspecialchars($row['caprino_etapa'] ?? '').'" 
                                                  data-producto="'.htmlspecialchars($row['ch_concentrado_producto'] ?? '').'" 
                                                  data-racion="'.htmlspecialchars($row['ch_concentrado_racion'] ?? '').'" 
                                                  data-costo="'.htmlspecialchars($row['ch_concentrado_costo'] ?? '').'" 
                                                  data-fecha-inicio="'.htmlspecialchars($concentradoFechaInicio ?? '').'"
                                                  data-fecha-fin="'.htmlspecialchars($concentradoFechaFin ?? '').'"
                                                  title="Editar Registro"
                                                  data-debug="DB: '.htmlspecialchars($row['ch_concentrado_producto'] ?? 'NULL').'">
                              ...
                              echo "<td>" . htmlspecialchars($row['caprino_tagid'] ?? 'N/A') . "</td>";
                              // Column 5: Etapa (or N/A)
                              echo "<td>" . htmlspecialchars($row['caprino_etapa'] ?? 'N/A') . "</td>";
                              // Column 6: Producto (or N/A)
                              echo "<td>" . ($hasConcentrado ? htmlspecialchars($row['ch_concentrado_producto'] ?? '') : 'N/A') . "</td>";
                              // Column 7: Racion (or N/A)
                              echo "<td>" . ($hasConcentrado ? htmlspecialchars($row['ch_concentrado_racion'] ?? '') : 'N/A') . "</td>";
                              // Column 8: Costo (or N/A)
                              echo "<td>" . ($hasConcentrado ? htmlspecialchars($row['ch_concentrado_costo'] ?? '') : 'N/A') . "</td>";
                        ...
                        <div class="mb-3">
                            ...
                            <div class="list-group" id="productList" style="max-height: 300px; overflow-y: auto;">
                                ...
                                try {
                                    ...
                                    $alimentos = $stmt_alimentos->fetchAll(PDO::FETCH_ASSOC);
                                    foreach ($alimentos as $alimento_row) {
                                        echo '<button type="button" class="list-group-item list-group-item-action product-option" data-value="' . htmlspecialchars($alimento_row['cc_concentrado_nombre']) . '">' . htmlspecialchars($alimento_row['cc_concentrado_nombre']) . '</button>';
                                    }
                                } catch (PDOException $e) {
                                    error_log("Error fetching alimentos: " . $e->getMessage());
                        ...
                        <div class="mb-3">
                            ...
                            <div class="list-group" id="etapaList" style="max-height: 300px; overflow-y: auto;">
                                ...
                                try {
                                    ...
                                    $etapas = $stmt_etapas->fetchAll(PDO::FETCH_ASSOC);
                                    foreach ($etapas as $etapa_row) {
                                        echo '<button type="button" class="list-group-item list-group-item-action etapa-option" data-value="' . htmlspecialchars($etapa_row['cc_etapas_nombre']) . '">' . htmlspecialchars($etapa_row['cc_etapas_nombre']) . '</button>';
                                    }
                                } catch (PDOException $e) {
                                    error_log("Error fetching etapas: " . $e->getMessage());
